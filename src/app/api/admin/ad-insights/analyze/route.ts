...
export async function POST(request: Request) {
  ...
  try {
    ...
    await recordActivity({ session, action: "Oluşturma", entity: "Reklam Yorum Merkezi", companyId, details: { message: "AI reklam yorumu oluşturuldu", result: "Başarılı" } }).catch(() => null);
    return NextResponse.json(data);
  } catch (error) {
    const safe = getSafeSupabaseError(error);
    await recordActionFailure({ session, entity: "Reklam Yorum Merkezi", action: "AI yorum oluşturma", error, companyId }).catch(() => null);
