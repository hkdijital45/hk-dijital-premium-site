...
import { runContractSlaMonitor } from "@/lib/contract-sla-monitor";
...
async function authorizeManualOrCron(request: Request): Promise<"cron" | "manual" | null> {
  ...
  const session = await requireModuleAccess("belgeler");
...
async function run(request: Request) {
  ...
  try {
    return NextResponse.json(await runContractSlaMonitor(mode));
  } catch (error) {
