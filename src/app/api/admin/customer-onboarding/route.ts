...
export async function POST(request: Request) {
  ...
  try {
    ...
    const brandingPayload = {
      ...
      logo_light_url: String(body.branding?.logo_light_url || "").trim(),
      logo_dark_url: String(body.branding?.logo_dark_url || "").trim(),
      primary_color: String(body.branding?.primary_color || "#22d3ee"),
      secondary_color: String(body.branding?.secondary_color || "#2563eb"),
      brand_accent_color: String(body.branding?.brand_accent_color || "#f59e0b"),
      brand_font_heading: String(body.branding?.brand_font_heading || "Inter").trim(),
      brand_font_body: String(body.branding?.brand_font_body || "Inter").trim(),
      social_profile_image_url: String(body.branding?.social_profile_image_url || "").trim(),
      email_signature_html: String(body.branding?.email_signature_html || "").trim(),
      letterhead_url: String(body.branding?.letterhead_url || "").trim(),
      brand_notes: String(body.branding?.brand_notes || "").trim(),
      brand_assets: body.branding?.brand_assets || {},
