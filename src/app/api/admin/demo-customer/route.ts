...
async function ensureReports(companyId: string, campaignId: string) {
  ...
  diagnosticLog("Eksik demo raporları oluşturuluyor", {
    ...
    reports: samples.map((sample) => ({ report_type: sample.report_type, keys: Object.keys(sample).sort() }))
  });
  diagnosticLog("Demo rapor payload", { reports: samples });
  let created;
  try {
    created = await supabaseRest<any[]>("reports", {
      method: "POST",
      body: JSON.stringify(samples)
    });
  } catch (error) {
    console.error(`${logPrefix} Demo rapor toplu ekleme başarısız`, {
      count: samples.length,
      reports: samples.map((sample) => ({ report_type: sample.report_type, payload: sample })),
