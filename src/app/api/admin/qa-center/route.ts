...
function sourceContains(pattern: string) {
  const files = [
    ...
    "src/app/api/admin/customer-integrations/credentials/route.ts",
    "src/app/api/customer/integrations/route.ts",
    "src/lib/customer-integration-oauth.ts",
    "src/app/api/customer/integrations/oauth/meta/start/route.ts",
    "src/app/api/customer/integrations/oauth/google/start/route.ts",
    "src/app/api/customer/integrations/oauth/tiktok/start/route.ts",
...
function scanSourcesForFindings(migrations: string) {
  ...
  [
    ...
    ["Müşteri panelinde Hesap Bağla merkezi var mı?", "Hesap Bağla", "Müşteri paneli Meta, Instagram, TikTok, Google Ads, Google Analytics ve Website/Pixel bilgilerini müşteri tarafından girilebilir göstermeli."],
    ["Hesap Bağla iki modlu mu?", "Otomatik Bağlan", "Müşteri platform kartında Otomatik Bağlan ve Manuel Bilgi Gir seçeneklerini görmeli."],
    ["OAuth hazırlık endpointleri kontrollü mü?", "oauth_not_configured", "Meta, Google ve TikTok OAuth env eksikse 500 yerine kontrollü hazırlık/501 yanıtı dönmeli."],
    ["Bağlanacak Hesapları Seç modalı var mı?", "Bağlanacak Hesapları Seç", "Otomatik bağlantı sonrası yetkili hesap seçimi tablo/checkbox görünümüyle hazırlanmalı."],
    ["Token düz metin saklanıyor mu?", "Gerçek access_token / refresh_token düz metin saklama", "OAuth hazırlık katmanı token kolonları eklememeli; yalnız durum ve asset metadata saklanmalı."],
    ["Müşteri entegrasyon endpointi hassas veriyi maskeliyor mu?", "maskSecret", "Müşteri API response içinde şifre ve erişim notları açık dönmemeli."],
    ["Admin-only bağlantı bilgileri endpointi var mı?", "customer-integrations/credentials", "Admin/staff kullanıcılar müşteri bağlantı bilgilerini özel endpoint üzerinden görmeli."],
    ["Admin entegrasyon merkezi müşteri kayıtlarını gösteriyor mu?", "Müşteri Bazlı Bağlantılar", "Admin Entegrasyonlar ekranı seçili müşterinin manuel/OAuth hazırlık bağlantılarını göstermeli."],
    ["Admin müşteri profilinde Bağlantı Bilgileri sekmesi var mı?", "Bağlantı Bilgileri", "Müşteri profilinde admin-only bağlantı bilgileri sekmesi bulunmalı."],
    ...
    ["Seçilenler İçin Teklif Hazırla CRM durumunu güncelliyor mu?", "Seçilen Leadler İçin Teklif Hazırlandı", "Seçili işletmeler için teklif taslağı oluşturulup lead aşaması Teklif Hazırlandı olarak işaretlenmelidir."],
    ["Teklif Merkezi’nde keşif teklifleri görünüyor mu?", "Google Maps / Müşteri Keşfi", "Teklif Takip Merkezi kaynak filtresiyle Google Maps/Müşteri Keşfi tekliflerini göstermelidir."],
    ["Google Maps sonuç kartında CRM’e Kaydet butonu var mı?", "CRM’e Kaydet", "Her işletme kartında tekil CRM kaydı için görünür kaydetme aksiyonu bulunmalıdır."],
    ["Sağ detay panelinde CRM’e Kaydet butonu var mı?", "CRM Kaydını Aç", "Seçilen işletme detayında kayıtlıysa CRM Kaydını Aç, değilse CRM’e Kaydet aksiyonu görünmelidir."],
    ["Seçilenleri CRM’e Kaydet toplu aksiyonu var mı?", "Seçilenleri CRM’e Kaydet", "Checkbox ile seçilen işletmeler toplu olarak CRM’e aktarılabilmelidir."],
    ["Kaydedilenler sekmesi gerçek kayıt gösteriyor mu?", "Kaydedilen CRM Leadleri", "Kaydedilenler sekmesi CRM’e aktarılan işletmelerin kaynak, firma, telefon, website, şehir/ilçe, durum ve teklif bilgilerini göstermelidir."],
    ["Duplicate kayıt engelleniyor mu?", "knownNameDistricts", "Business discovery API, place id, telefon, website ve firma+ilçe eşleşmesiyle tekrar kayıt açmayı engellemelidir."],
    ["Kaydedildi rozeti doğru çalışıyor mu?", "CRM’de Kayıtlı", "Kayıtlı işletmelerde CRM’e Kaydet yerine kayıt rozeti ve CRM Kaydını Aç aksiyonu görünmelidir."],
    ["Teklif Hazırla CRM kaydıyla bağlantılı mı?", "Teklif hazırlamak için CRM kaydı gerekli", "Teklif hazırlama akışı kayıt yoksa önce CRM kaydı oluşturmalı veya kullanıcıya açık uyarı göstermelidir."],
    ["AI İlk Mesaj Hazırla çalışıyor mu?", "AI İlk Mesaj Hazırlandı", "Lead kartı veya detay paneli WhatsApp, Instagram DM, telefon/e-posta temasına uygun mesaj taslağı üretmelidir."],
    ["AI SWOT Raporu oluşuyor mu?", "AI SWOT Raporu Hazırlandı", "İşletme için güçlü/zayıf yön, fırsat, tehdit ve aksiyon planı içeren rapor ActionResultPanel ile gösterilmelidir."],
    ["Rapor Sun butonu var mı?", "AI Sunum Hazırlandı", "Dijital analiz raporu print-ready HTML/PDF uyumlu sunum taslağına çevrilebilmelidir."],
    ["WhatsApp/Instagram iletişim butonları var mı?", "Instagram DM Hazırla", "İşletme kartı ve detay panelinde WhatsApp, Instagram, arama, e-posta, website ve Maps iletişim aksiyonları bulunmalıdır."],
    ...
    ["HK Digital OS v2.0 rakip canlı panelleri var mı?", "Rakip İstihbarat Merkezi", "Rakip Merkezi bulunan rakipler, takip edilen rakipler, yeni sinyaller ve müşteriye özet akışlarını aynı ekranda göstermelidir."],
    ...
    ["Muhasebe modülleri doğru kategoride mi?", "label: \"Muhasebe\"", "Tahsilat, bekleyen ödeme, gelir/gider, gelir tahmini ve kârlılık Muhasebe kategorisinde görünmelidir."],
    ["Gelir tahmini Muhasebe altında mı?", "Gelir Tahmini", "Gelir Tahmini finansal modüldür; Ajans Operasyonu veya Dashboard altında görünmemelidir."],
    ["Bekleyen ödemeler Muhasebe altında mı?", "Bekleyen Ödemeler", "Bekleyen Ödemeler Muhasebe kategorisinde bağımsız modül bağlantısı olarak bulunmalıdır."],
    ...
    ["Admin mimarisi kategori bazlı mı?", "HK Admin Mimarisi", "QA Center modüllerin Dashboard içine gömülmediğini ve profesyonel kategorilerde kaldığını kontrol etmelidir."],
    ["Dashboard içine yanlış taşınmış modül var mı?", "AdminModuleCenter", "AdminDashboard içinde tek merkez component ile modül gömme sinyali bulunmamalıdır."],
    ["Müşteri profilinde sekmeler en üstte mi?", "profileTabs", "Müşteri profili popup açıldığında önce sekme/chip navigasyonu ve seçili sekmenin temel bilgileri görünmelidir."],
    ["Şubeler ve Rakipler en üstte değil mi?", "Operasyon Detayları", "Şubeler, Rakipler ve Uygulanan Paketler temel müşteri bilgileri altında Operasyon Detayları bölümünde yer almalıdır."],
    ["Müşteri profili modal genişliği okunabilir mi?", "sm:w-[min(1200px,94vw)]", "Müşteri profili modalı desktop üzerinde dar kalmamalı, kendi içinde scroll etmelidir."],
    ["Dropdown itemleri dar kartlara sıkışıyor mu?", "xl:grid-cols-2", "Admin dropdown en fazla iki kolon kullanmalı; üç kolonlu sıkışık kart düzenine dönmemelidir."],
    ["Ayarlar sayfaları Kontrol Merkezi’ne yanlış konmuş mu?", "label: \"Ayarlar\"", "Web sitesi, kullanıcı yönetimi, roller, tema, sistem ayarları, mobil operasyon ve güvenlik Ayarlar kategorisinde kalmalıdır."],
    ["Dropdown aynı anda birden fazla kategori açıyor mu?", "Object.fromEntries(adminNavigationGroups.map", "Dropdown açılışı tek açık kategori mantığıyla çalışmalı; yeni kategori açılınca diğerleri kapanmalıdır."],
    ["Dropdown dışına tıklayınca kapanıyor mu?", "handlePointerDown", "Admin menüsü dışına tıklanınca açık dropdown kapanmalıdır."],
    ["Escape ile menü kapanıyor mu?", "handleKeyDown", "Escape tuşu açık admin dropdown menüsünü kapatmalıdır."],
