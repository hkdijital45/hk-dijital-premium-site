...
export async function GET(request: Request) {
  ...
  try {
    const [leads, companies, reports] = await Promise.all([
    ...
    leads.filter((lead) => includes(`${lead.name} ${lead.company} ${lead.phone} ${lead.email} ${lead.notes}`, query)).slice(0, 8).forEach((lead) => results.push({ id: `lead-${lead.id}`, type: "Lead", title: lead.company || lead.name || "İsimsiz başvuru", detail: lead.phone || lead.email || lead.status, href: "/hk-admin/leads" }));
