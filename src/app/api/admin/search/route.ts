...
export async function GET(request: Request) {
  ...
  const results: any[] = adminNavigationItems
    ...
    .map((item) => ({ id: `module-${item.module}`, type: "Modül", title: item.label, detail: "Yönetim modülü", href: getAdminHref(item.slug) }));
  try {
    const [leads, companies, reports, users] = await Promise.all([
      ...
      allowed.includes("kullanicilar") ? supabaseRest<any[]>("users?select=id,full_name,email,role,is_active&order=created_at.desc&limit=100") : []
    ]);
    ...
    users.filter((user) => includes(`${user.full_name} ${user.email} ${user.role}`, query)).slice(0, 8).forEach((user) => results.push({ id: `user-${user.id}`, type: "Kullanıcı", title: user.full_name || user.email, detail: `${user.email} · ${user.role}`, href: "/hk-admin/kullanicilar" }));
  } catch (error) {
