...
export async function GET(request: Request) {
  ...
  try {
    ...
    filterSelectableCustomers(companies).filter((company) => includes(`${company.name} ${company.sector} ${company.city} ${company.notes}`, query)).slice(0, 8).forEach((company) => results.push({ id: `company-${company.id}`, type: "Müşteri", title: company.name, detail: `${company.sector || "Sektör yok"} · ${company.city || "Şehir yok"}`, href: "/hk-admin/musteriler" }));
