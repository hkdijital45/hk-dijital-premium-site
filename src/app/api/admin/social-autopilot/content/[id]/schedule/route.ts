...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (typeof body.scheduled_at !== "string") return NextResponse.json({ error: "scheduled_at zorunludur." }, { status: 400 });
  try {
    return NextResponse.json({ result: await execute("instagram_schedule_post", { id, scheduled_at: body.scheduled_at }) });
  } catch (error) {
