...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const message = await supabaseRest<Array<{ id: string }>>(`customer_messages?id=eq.${messageId}&conversation_id=eq.${id}&select=id&limit=1`);
    ...
    const storagePath = `${conversation.company_id}/${id}/${crypto.randomUUID()}.${extension}`;
    ...
    try {
      const rows = await supabaseRest<Array<{ id: string }>>("conversation_attachments", {
        method: "POST",
        body: JSON.stringify({
          conversation_id: id,
