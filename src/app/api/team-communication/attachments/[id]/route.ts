...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const rows = await supabaseRest<Array<{ conversation_id: string; storage_bucket: string; storage_path: string }>>(
    `team_attachments?id=eq.${id}&select=conversation_id,storage_bucket,storage_path&limit=1`
  ).catch(() => []);
  const attachment = rows[0];
  if (!attachment || !await getAccessibleTeamConversation(context, attachment.conversation_id)) {
