...
export function ActionResultPanel({ result, onNavigate, compact = false }: { result: ActionResult; onNavigate?: (link: string) => void; compact?: boolean }) {
  ...
  return (
    <section className={`rounded-[18px] border border-cyan-200 bg-cyan-50 p-4 text-[var(--admin-text-primary)] ${compact ? "" : "shadow-sm"}`}>
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="text-xs font-black uppercase tracking-[.14em] text-cyan-700">İşlem Sonucu Rehberi</p>
          <h3 className="mt-1 text-lg font-black text-[var(--admin-text-primary)]">{result.title || "İşlem tamamlandı"}</h3>
      ...
      <div className="mt-4 grid gap-3 lg:grid-cols-3">
        <div className="rounded-[14px] border border-cyan-200 bg-[var(--admin-surface)] p-3">
          <div className="flex items-center gap-2 text-sm font-black text-[var(--admin-text-primary)]"><Info size={16} /> Ne oldu?</div>
          <div className="mt-2 grid gap-1 text-xs leading-5 text-[var(--admin-text-secondary)]">
        ...
        <div className="rounded-[14px] border border-cyan-200 bg-[var(--admin-surface)] p-3">
          <div className="flex items-center gap-2 text-sm font-black text-[var(--admin-text-primary)]"><CheckCircle2 size={16} /> Oluşan kayıtlar</div>
          <div className="mt-2 grid gap-2">
            {createdRecords.length ? createdRecords.map((record) => (
              <div key={`${record.label}-${record.status}`} className="flex items-center justify-between gap-2 rounded-[10px] bg-[var(--admin-surface-soft)] px-3 py-2 text-xs">
                <span className="font-bold text-[var(--admin-text-secondary)]">{record.label}</span>
                <span className="rounded-full bg-[var(--admin-surface)] px-2 py-1 font-black text-cyan-700 ring-1 ring-cyan-100">{record.count ?? 1} · {record.status || "Hazır"}</span>
              </div>
            )) : <p className="text-xs text-[var(--admin-text-muted)]">Bu işlem yeni kayıt oluşturmadan mevcut kaydı güncelledi.</p>}
          </div>
        </div>
        <div className="rounded-[14px] border border-cyan-200 bg-[var(--admin-surface)] p-3">
          <div className="flex items-center gap-2 text-sm font-black text-[var(--admin-text-primary)]"><ShieldAlert size={16} /> Şimdi ne yapmalısın?</div>
          <ol className="mt-2 grid list-decimal gap-1 pl-4 text-xs leading-5 text-[var(--admin-text-secondary)]">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {checkLinks.map((link) => (
            <button
              ...
              className="inline-flex items-center gap-2 rounded-[10px] border border-cyan-200 bg-[var(--admin-surface)] px-3 py-2 text-xs font-black text-cyan-700"
            >
