...
function OperationRecords({ companyId, resource }: { companyId: string; resource: "payment" | "task" }) {
  ...
  return <div className="grid gap-5">
    ...
    <section aria-busy={loading} aria-live="polite" className="grid gap-3">
      {loading ? <LoadingRows /> : items.map((item) => <article key={item.id} className="rounded-[12px] border border-slate-200 bg-white p-4 shadow-sm"><div className="flex flex-wrap items-center justify-between gap-3"><strong className="text-slate-950">{payment ? `${Number(item.amount || 0).toLocaleString("tr-TR")} TL` : item.title}</strong><StatusBadge status={item.status} /></div><p className="mt-2 text-sm leading-6 text-slate-600">{item.description || "Açıklama eklenmemiş."}</p><p className="mt-1 text-xs font-semibold text-slate-500">{item.due_date ? `Tarih: ${new Date(`${item.due_date}T00:00:00`).toLocaleDateString("tr-TR")}` : "Tarih tanımlanmamış."}</p></article>)}
