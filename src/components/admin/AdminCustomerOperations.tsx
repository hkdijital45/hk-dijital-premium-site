    ...
    <div className="rounded-[14px] border border-cyan-200 bg-cyan-50 p-3">
      <div className="grid gap-3 lg:grid-cols-[minmax(240px,1fr)_auto] lg:items-end">
        <label className="grid gap-2 text-xs font-black uppercase tracking-[.12em] text-cyan-800">
          Müşteri filtresi
          <select value={value} onChange={(event) => onChange(event.target.value)} className="min-h-11 rounded-[10px] border border-slate-300 bg-white px-3 text-sm font-bold normal-case tracking-normal text-slate-900 outline-none focus:border-cyan-500 focus:ring-2 focus:ring-cyan-100">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <button type="button" onClick={onClear} className="min-h-11 rounded-[10px] border border-slate-300 bg-white px-5 text-sm font-black text-slate-700 transition hover:bg-slate-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-400 focus-visible:ring-offset-2">Temizle</button>
        </div>
      </div>
      <div className="mt-2 flex flex-wrap items-center justify-between gap-2">
        <p className="text-xs leading-5 text-slate-600">Seçim, Filtrele düğmesine basıldığında uygulanır ve bu tarayıcıda hatırlanır.</p>
...
export function GlobalMetaPixelSettings() {
  ...
  return <div className="grid gap-4 rounded-[14px] border border-slate-200 bg-white p-4"><div><h3 className="font-black text-slate-900">Genel Web Sitesi Meta Pixel</h3><p className="mt-1 text-sm text-slate-600">Panelde kayıtlı değer varsa ortam değişkenindeki genel Pixel kimliğinden önce kullanılır.</p></div><Input label="Genel Meta Pixel Kimliği" value={form.pixel_id} onChange={(pixel_id) => setForm({ ...form, pixel_id })} /><Toggle label="Web sitesinde aktif" checked={form.enabled} onChange={(enabled) => setForm({ ...form, enabled })} />{notice && <NoticeMessage notice={notice} />}<Action disabled={saving} onClick={save} tone="green"><Save size={16} /> {saving ? "Kaydediliyor..." : "Ayarları Kaydet"}</Action></div>;
}
...
export function CustomerWorkflowLinks({ companyId }: { companyId: string }) {
  ...
  return <nav aria-label="İlgili müşteri işlemleri" className="rounded-[14px] border border-slate-200 bg-slate-50 p-4">
    <div className="flex flex-wrap items-center justify-between gap-2"><div><h3 className="text-sm font-black text-slate-900">İlgili İşlemler</h3><p className="mt-1 text-xs text-slate-600">Seçili müşteri bağlamını kaybetmeden iş akışına devam edin.</p></div><Link href={`/musteri-paneli?company=${companyId}`} target="_blank" rel="noreferrer" className="inline-flex min-h-10 items-center gap-2 rounded-[10px] border border-cyan-200 bg-white px-3 text-xs font-black text-cyan-800 hover:bg-cyan-50">Müşteri panelini aç <ArrowRight size={14} /></Link></div>
    <div className="mt-3 flex flex-wrap gap-2">{links.map(({ label, href, icon: Icon }) => <Link key={label} href={href} className="inline-flex min-h-10 items-center gap-2 rounded-[10px] border border-slate-200 bg-white px-3 text-xs font-black text-slate-700 transition hover:border-cyan-300 hover:text-cyan-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-500"><Icon size={15} />{label}</Link>)}</div>
  </nav>;
}
...
function OperationRecords({ companyId, resource }: { companyId: string; resource: "payment" | "task" }) {
  ...
  return <div className="grid gap-5">
    ...
    {notice && <NoticeMessage notice={notice} />}
    <div className="flex flex-wrap justify-end gap-2 border-t border-slate-200 pt-4"><Action disabled={saving} onClick={() => { setForm(payment ? emptyPayment : emptyTask); setNotice(null); }} tone="neutral"><RotateCcw size={16} /> Formu Temizle</Action><Action disabled={saving} onClick={save} tone="green">{saving ? <LoaderCircle className="animate-spin" size={16} /> : payment ? <CreditCard size={16} /> : <ListChecks size={16} />} {saving ? "Kaydediliyor..." : payment ? "Tahsilat Ekle" : "Görev Ekle"}</Action></div>
    <section aria-busy={loading} aria-live="polite" className="grid gap-3">
      {loading ? <LoadingRows /> : items.map((item) => <article key={item.id} className="rounded-[12px] border border-slate-200 bg-white p-4 shadow-sm"><div className="flex flex-wrap items-center justify-between gap-3"><strong className="text-slate-950">{payment ? `${Number(item.amount || 0).toLocaleString("tr-TR")} TL` : item.title}</strong><StatusBadge status={item.status} /></div><p className="mt-2 text-sm leading-6 text-slate-600">{item.description || item.payment_note || "Açıklama eklenmemiş."}</p><p className="mt-1 text-xs font-semibold text-slate-500">{item.due_date ? `Tarih: ${new Date(`${item.due_date}T00:00:00`).toLocaleDateString("tr-TR")}` : "Tarih tanımlanmamış."}</p></article>)}
...
function EmptyState({ title, description, actionLabel, onAction, icon }: { title: string; description: string; actionLabel?: string; onAction?: () => void; icon: ReactNode }) { return <div className="rounded-[14px] border border-dashed border-cyan-300 bg-cyan-50 p-7 text-center text-slate-900"><span className="mx-auto grid size-11 place-items-center rounded-[12px] bg-white text-cyan-700 shadow-sm">{icon}</span><p className="mt-3 font-black">{title}</p><p className="mx-auto mt-2 max-w-lg text-sm leading-6 text-slate-600">{description}</p>{actionLabel && onAction && <button type="button" onClick={onAction} className="mt-4 min-h-11 rounded-[10px] bg-cyan-600 px-4 text-sm font-black text-white hover:bg-cyan-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-500 focus-visible:ring-offset-2">{actionLabel}</button>}</div>; }
function LoadingRows() { return <>{[1, 2].map((item) => <div key={item} className="animate-pulse rounded-[12px] border border-slate-200 bg-white p-4"><div className="h-4 w-1/3 rounded bg-slate-200" /><div className="mt-3 h-3 w-3/4 rounded bg-slate-100" /></div>)}</>; }
...
function Input({ id, label, value, onChange, type = "text", placeholder = "", help = "" }: { id?: string; label: string; value: string | number | null | undefined; onChange: (value: string) => void; type?: string; placeholder?: string; help?: string }) { return <label className="grid gap-2 text-sm font-bold text-slate-700">{label}<input id={id} type={type} value={value ?? ""} placeholder={placeholder} onChange={(event) => onChange(event.target.value)} className="min-h-11 rounded-[8px] border border-slate-300 bg-white px-3 text-slate-900 outline-none placeholder:text-slate-500 focus:border-cyan-500 focus:ring-2 focus:ring-cyan-100" />{help && <span className="text-xs font-normal leading-5 text-slate-500">{help}</span>}</label>; }
function Select({ label, value, options, onChange }: { label: string; value: string; options: string[]; onChange: (value: string) => void }) { return <label className="grid gap-2 text-sm font-bold text-slate-700">{label}<select value={value} onChange={(event) => onChange(event.target.value)} className="min-h-11 rounded-[8px] border border-slate-300 bg-white px-3 text-slate-900 outline-none focus:border-cyan-500 focus:ring-2 focus:ring-cyan-100">{options.map((option) => <option key={option}>{option}</option>)}</select></label>; }
function Toggle({ label, checked, onChange }: { label: string; checked: boolean; onChange: (value: boolean) => void }) { return <label className="flex min-h-11 items-center gap-3 rounded-[8px] border border-slate-200 bg-white p-3 text-sm font-bold text-slate-700 focus-within:ring-2 focus-within:ring-cyan-100"><input type="checkbox" checked={checked} onChange={(event) => onChange(event.target.checked)} className="size-4 accent-cyan-600" /> {label}</label>; }
...
function Action({ children, onClick, disabled = false, tone = "cyan" }: { children: ReactNode; onClick: () => void; disabled?: boolean; tone?: "cyan" | "blue" | "green" | "orange" | "neutral" }) { const colors = { cyan: "bg-cyan-600 text-white hover:bg-cyan-700", blue: "bg-blue-600 text-white hover:bg-blue-700", green: "bg-emerald-600 text-white hover:bg-emerald-700", orange: "bg-orange-600 text-white hover:bg-orange-700", neutral: "border border-slate-300 bg-white text-slate-700 hover:bg-slate-50" }; return <button type="button" disabled={disabled} onClick={onClick} className={`inline-flex min-h-11 w-fit items-center justify-center gap-2 rounded-[10px] px-4 text-sm font-black transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-500 focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 ${colors[tone]}`}>{children}</button>; }
