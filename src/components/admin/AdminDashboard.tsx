...
function SocialMediaAuditCenter() {
  ...
  async function scanProfile(index) {
    ...
    try {
      ...
      setMessage(data.success ? "Profil bilgileri alındı." : data.warning || "Instagram bazı profil verilerini engelleyebilir. Ekran görüntüsü yüklerseniz AI daha doğru analiz yapar.");
    } catch {
      setPlatforms((items) => items.map((item, itemIndex) => itemIndex === index ? {
        ...item,
        fetchStatus: "Alınamadı",
        fetchWarning: "Instagram bazı profil verilerini engelleyebilir. Ekran görüntüsü yüklerseniz AI daha doğru analiz yapar."
      } : item));
      setMessage("Instagram bazı profil verilerini engelleyebilir. Ekran görüntüsü yüklerseniz AI daha doğru analiz yapar.");
    } finally {
