...
function AiProviderChooserModal({ open, selected, setSelected, onContinue, onCancel }: any) {
  if (!open) return null;
  return (
    <div className="fixed inset-0 z-[80] grid place-items-center bg-slate-950/75 p-4 backdrop-blur-xl">
      <div className="w-full max-w-xl rounded-[8px] border border-white/10 bg-[#071023] p-5 text-white shadow-[0_24px_90px_rgba(0,0,0,.45)]">
        ...
        <div className="mt-5 grid gap-2">
          {aiChooserOptions.map(([label, description]) => (
            <button
              ...
              className={`rounded-[8px] border p-4 text-left transition ${selected === label ? "border-amber-200 bg-amber-300 text-slate-950 shadow-[0_16px_45px_rgba(251,191,36,.18)]" : "border-white/10 bg-white/[0.045] hover:border-cyan-200/30 hover:bg-cyan-200/[0.07]"}`}
            >
        ...
        <div className="mt-5 flex flex-wrap justify-end gap-2">
          <button onClick={onCancel} className="rounded-full border border-white/10 px-5 py-2 text-sm font-bold text-slate-200">Vazgeç</button>
...
function Panel({ title, children }: any) {
  return <div className="w-full min-w-0 max-w-none"><p className="text-[10px] font-black uppercase tracking-[.2em] text-cyan-200">HK Operating System</p><h2 className="mb-6 mt-2 text-2xl font-black">{title}</h2>{children}</div>;
}

function Field({ label, value, onChange, type = "text", placeholder = "" }: any) {
  return <label className="grid gap-2 text-sm font-semibold text-slate-200">{label}<input type={type} value={value ?? ""} onChange={(e) => onChange(e.target.value)} placeholder={placeholder} className="min-h-11 rounded-[8px] border border-white/10 bg-black/30 px-3 text-white placeholder:text-slate-500" /></label>;
}
...
function StartupApiStatusModal({ open, loading, data = {}, message, onRetest, onClose, onSettings }: any) {
  ...
  return (
    <div className="fixed inset-0 z-[75] grid place-items-center bg-slate-950/70 p-4 backdrop-blur-xl">
      <div className="w-full max-w-2xl rounded-[8px] border border-white/10 bg-[#071023] p-5 text-white shadow-[0_24px_90px_rgba(0,0,0,.45)]">
        ...
        {message && <p className={`mt-4 rounded-[8px] border p-3 text-sm ${message.includes("Sistem hazır") ? "border-emerald-300/25 bg-emerald-300/10 text-emerald-100" : "border-amber-300/25 bg-amber-300/10 text-amber-100"}`}>{message}</p>}
        <div className="mt-5 flex flex-wrap justify-end gap-2">
          <button disabled={loading} onClick={onRetest} className="rounded-full border border-cyan-200/20 px-5 py-2 text-sm font-bold text-cyan-100 disabled:opacity-60">{loading ? "Test Ediliyor" : "Yeniden Test Et"}</button>
          <button onClick={onSettings} className="rounded-full border border-white/10 px-5 py-2 text-sm font-bold text-slate-200">API Ayarlarına Git</button>
...
function ReadinessPanel({ api }: any) {
  ...
  return <div className="rounded-[8px] border border-white/10 bg-black/15 p-4">
    <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
      <div><p className="text-xs font-black uppercase tracking-[.14em] text-cyan-200">Meta & Google hazırlık kontrolü</p><h3 className="mt-1 font-black text-white">Canlı veri bağlantısı için gerekli alanlar</h3></div>
      <span className="rounded-full border border-white/10 px-3 py-1 text-xs text-slate-300">{items.filter((item) => item.ready).length}/{items.length} hazır</span>
    </div>
    <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
      {items.map((item) => <div key={item.label} className={`rounded-[8px] border p-3 ${item.ready ? "border-emerald-300/20 bg-emerald-300/10 text-emerald-100" : "border-amber-300/20 bg-amber-300/10 text-amber-100"}`}>
        <div className="flex items-center justify-between gap-3"><p className="text-sm font-black text-white">{item.label}</p><span className="rounded-full border border-current/20 px-2 py-1 text-[10px] font-black">{item.ready ? "Hazır" : "Eksik"}</span></div>
...
function AgencyStatCard({ label, value, note, tone = "cyan" }: any) {
  const toneClass = tone === "red" ? "border-red-300/20 bg-red-300/10 text-red-100" : tone === "emerald" ? "border-emerald-300/20 bg-emerald-300/10 text-emerald-100" : tone === "amber" ? "border-amber-300/20 bg-amber-300/10 text-amber-100" : "border-cyan-300/20 bg-cyan-300/10 text-cyan-100";
  return <div className={`rounded-[8px] border p-4 ${toneClass}`}><p className="text-xs font-black uppercase tracking-[.14em] opacity-80">{label}</p><p className="mt-2 text-2xl font-black text-white">{value}</p><p className="mt-1 text-xs leading-5 opacity-80">{note}</p></div>;
}
  ...
  return (
    <Panel title="Müşteri Markalama">
      ...
      <div className="grid gap-4">
        {items.map((item, index) => (
          <div key={item.id || index} className="rounded-[8px] border border-white/10 bg-black/15 p-4">
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <Field label="İkincil renk" type="color" value={item.secondary_color || "#0f172a"} onChange={(value) => update(index, { secondary_color: value })} />
        ...
        {!items.length && <p className="rounded-[8px] border border-dashed border-white/10 p-6 text-sm text-slate-400">Henüz müşteri markalaması yok.</p>}
      </div>
    </Panel>
  );
}
...
function CompetitorAnalysisCenter({ content, setContent }: any) {
  ...
  return <Panel title="Rakip Analizi"><div className="mb-4 flex flex-wrap items-center justify-between gap-3"><p className="text-sm text-slate-400">Rakipleri manuel girin veya demo rakip listesiyle güvenli analiz taslağı üretin.</p><button onClick={add} className="rounded-full bg-cyan-300 px-4 py-2 text-sm font-black text-slate-950">Rakip analizi ekle</button></div><div className="grid gap-4">{items.map((item, index) => <div key={item.id || index} className="rounded-[8px] border border-white/10 bg-black/15 p-4"><div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4"><CompanySelect value={item.company_id || ""} onChange={(value) => update(index, { company_id: value })} companies={content.companies} /><Field label="Sektör" value={item.sector || ""} onChange={(value) => update(index, { sector: value })} /><Field label="İl" value={item.city || ""} onChange={(value) => update(index, { city: value })} /><Field label="İlçe" value={item.district || ""} onChange={(value) => update(index, { district: value })} /></div><TextArea label="Rakipler JSON" value={JSON.stringify(item.competitors || [], null, 2)} onChange={(value) => { try { update(index, { competitors: JSON.parse(value || "[]") }); } catch {} }} /><div className="grid gap-3 md:grid-cols-3"><TextArea label="Rakip Özeti" value={item.ai_summary || ""} onChange={(value) => update(index, { ai_summary: value })} /><TextArea label="Fırsatlar" value={item.opportunities || ""} onChange={(value) => update(index, { opportunities: value })} /><TextArea label="Önerilen Aksiyonlar" value={item.recommended_actions || ""} onChange={(value) => update(index, { recommended_actions: value })} /></div><div className="mt-3 flex flex-wrap gap-2"><button disabled={busy === (item.id || `${index}`)} onClick={() => generate(index)} className="rounded-full border border-purple-200/30 px-4 py-2 text-xs font-black text-purple-100 disabled:opacity-60">AI rakip özeti üret</button><button onClick={() => update(index, { competitors: buildDemoCompetitors(item.sector, item.city) })} className="rounded-full border border-cyan-200/30 px-4 py-2 text-xs font-black text-cyan-100">Demo rakip listesi üret</button><button onClick={() => updateCollection(content, setContent, "competitorAnalyses", items.filter((_, i) => i !== index))} className="rounded-full border border-red-300/30 px-4 py-2 text-xs text-red-200">Sil</button></div></div>)}{!items.length && <p className="rounded-[8px] border border-dashed border-white/10 p-6 text-sm text-slate-400">Henüz rakip analizi yok.</p>}</div></Panel>;
}
...
function SocialPlanGenerator({ content, setContent }: any) {
  ...
  return <Panel title="Sosyal Medya Planı"><div className="mb-4 flex flex-wrap items-center justify-between gap-3"><p className="text-sm text-slate-400">Tek tıkla günlük içerik takvimi, caption fikri, görsel fikri, CTA (aksiyon çağrısı) ve hashtag önerileri üretin.</p><button onClick={add} className="rounded-full bg-cyan-300 px-4 py-2 text-sm font-black text-slate-950">Plan oluştur</button></div><div className="grid gap-4">{items.map((item, index) => <div key={item.id || index} className="rounded-[8px] border border-white/10 bg-black/15 p-4"><div className="grid gap-3 md:grid-cols-2 xl:grid-cols-5"><CompanySelect value={item.company_id || ""} onChange={(value) => update(index, { company_id: value })} companies={content.companies} /><Field label="Sektör" value={item.sector || ""} onChange={(value) => update(index, { sector: value })} /><SelectField label="Hedef" value={item.goal || "Bilinirlik"} onChange={(value) => update(index, { goal: value })} options={["Bilinirlik", "Mesaj", "Trafik", "Satış", "Randevu"]} /><SelectField label="Platform" value={item.platform || "Instagram"} onChange={(value) => update(index, { platform: value })} options={["Instagram", "Facebook", "LinkedIn", "TikTok"]} /><SelectField label="Süre" value={item.duration || "30 gün"} onChange={(value) => update(index, { duration: value })} options={["7 gün", "14 gün", "30 gün"]} /></div><div className="mt-3 flex flex-wrap gap-2"><button onClick={() => update(index, { plan_items: createSocialPlan(item) })} className="rounded-full border border-cyan-200/30 px-4 py-2 text-xs font-black text-cyan-100">Takvimi yenile</button><button onClick={() => updateCollection(content, setContent, "socialMediaPlans", items.filter((_, i) => i !== index))} className="rounded-full border border-red-300/30 px-4 py-2 text-xs text-red-200">Sil</button></div><div className="premium-scrollbar mt-4 max-h-[420px] overflow-y-auto rounded-[8px] border border-white/10"><table className="w-full min-w-[760px] text-left text-xs"><thead className="bg-white/[0.06] text-slate-300"><tr><th className="p-3">Gün</th><th>Tür</th><th>Caption fikri</th><th>Görsel fikri</th><th>CTA</th><th>Hashtag</th></tr></thead><tbody>{(item.plan_items || []).map((plan) => <tr key={`${item.id}-${plan.day}`} className="border-t border-white/10"><td className="p-3 font-black">{plan.day}</td><td>{plan.contentType}</td><td className="max-w-xs pr-3">{plan.caption}</td><td className="max-w-xs pr-3">{plan.visualIdea}</td><td>{plan.cta}</td><td>{(plan.hashtags || []).join(" ")}</td></tr>)}</tbody></table></div></div>)}{!items.length && <p className="rounded-[8px] border border-dashed border-white/10 p-6 text-sm text-slate-400">Henüz sosyal medya planı yok.</p>}</div></Panel>;
}
            ...
            {previewLead ? (
              <div className="mt-4">
                ...
                <div className="mt-4 grid gap-2">
                  ...
                  <button onClick={() => setSelectedLead(previewLead)} className="rounded-[8px] border border-purple-200/25 px-4 py-2 text-sm font-black text-purple-100">AI Analiz</button>
                  <button onClick={() => setActive("Teklif Hazırlama")} className="rounded-[8px] border border-amber-200/25 px-4 py-2 text-sm font-black text-amber-100">Teklif Hazırla</button>
                  <a href={previewLead.phone ? `https://wa.me/${String(previewLead.phone).replace(/\D/g, "")}` : "#"} target="_blank" rel="noreferrer" className="rounded-[8px] border border-emerald-200/25 px-4 py-2 text-center text-sm font-black text-emerald-100">WhatsApp</a>
                  <button onClick={() => setSelectedLead(previewLead)} className="rounded-[8px] border border-cyan-200/25 px-4 py-2 text-sm font-black text-cyan-100">PDF Audit</button>
                </div>
              </div>
            ) : (
...
function SalesPipeline({ content, setContent, save, setActive }: any) {
  ...
  return <Panel title="Satış Hunisi">
    <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
      ...
      <button onClick={() => setActive("CRM")} className="rounded-[8px] border border-cyan-200/20 px-4 py-3 text-sm font-black text-cyan-100">CRM Listesine Git</button>
    </div>
    {message && <p className="mb-4 rounded-[8px] border border-cyan-200/20 bg-cyan-200/10 p-3 text-sm text-cyan-100">{message}</p>}
    <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_360px]">
      <section className="premium-scrollbar grid gap-3 overflow-x-auto pb-2 xl:grid-cols-7">
        {salesPipelineStages.map((stage) => {
          ...
          return <div key={stage} className="min-w-[250px] rounded-[8px] border border-white/10 bg-black/15 p-3">
            <div className="mb-3 flex items-center justify-between gap-2">
              <h3 className="text-sm font-black text-white">{stage}</h3>
              <span className="rounded-full bg-white/10 px-2 py-1 text-[10px] font-black text-slate-300">{stageLeads.length}</span>
            </div>
            <div className="grid gap-2">
              {stageLeads.map((lead) => (
                <button key={lead.id} onClick={() => setSelectedLead(lead)} className="rounded-[8px] border border-white/10 bg-white/[0.035] p-3 text-left transition hover:border-cyan-200/35 hover:bg-cyan-200/[0.07]">
                  <strong className="block text-sm text-white">{lead.company || lead.name || "İsimsiz lead"}</strong>
                  ...
                  <span className="mt-2 flex flex-wrap gap-1 text-[10px] font-bold text-slate-300"><span className="rounded-full border border-orange-200/20 px-2 py-1">Skor {lead.lead_heat_score || 0}</span><span className="rounded-full border border-white/10 px-2 py-1">Son temas {formatDate(lead.last_contact_at || lead.updated_at)}</span></span>
                  <span className="mt-2 block text-[11px] leading-4 text-cyan-100">Sıradaki aksiyon: {lead.next_action || lead.next_action_at || "Takip planı girilmedi"}</span>
                </button>
              ))}
              {!stageLeads.length && <p className="rounded-[8px] border border-dashed border-white/10 p-3 text-xs leading-5 text-slate-500">Bu aşamada lead yok.</p>}
            </div>
          </div>;
        })}
      </section>
      <aside className="h-fit rounded-[8px] border border-white/10 bg-white/[0.04] p-4">
        <p className="text-xs font-black uppercase tracking-[.14em] text-cyan-200">Lead aksiyon merkezi</p>
        {selectedLead ? <div className="mt-4">
          <h3 className="text-lg font-black text-white">{selectedLead.company || selectedLead.name}</h3>
          ...
          <div className="mt-4 grid gap-3">
            ...
            <button onClick={addNote} className="rounded-[8px] border border-white/10 px-4 py-2 text-sm font-black text-slate-100">Not Ekle</button>
          </div>
          <ContactActionCenter record={selectedLead} type="lead" context="follow-up" />
          <div className="mt-4 grid gap-2">
            <button onClick={() => setActive("Teklif Hazırlama")} className="rounded-[8px] border border-amber-200/25 px-4 py-3 text-sm font-black text-amber-100">Teklif Oluştur</button>
            <button onClick={() => setActive("CRM")} className="rounded-[8px] border border-cyan-200/25 px-4 py-3 text-sm font-black text-cyan-100">CRM Detayına Git</button>
          </div>
        </div> : <p className="mt-4 rounded-[8px] border border-dashed border-white/10 p-4 text-sm leading-6 text-slate-400">Aksiyon almak için bir lead kartı seçin.</p>}
      </aside>
    </div>
  </Panel>;
}
...
function ContactActionCenter({ record, type = "lead", context = "new-lead" }: any) {
  ...
  return <div className="mt-4 rounded-[8px] border border-emerald-200/20 bg-emerald-300/[0.06] p-4">
    <div className="flex flex-wrap items-center justify-between gap-3">
      <div><p className="text-xs font-black uppercase tracking-[.14em] text-emerald-200">WhatsApp / İletişim Merkezi</p><p className="mt-1 text-sm text-slate-300">{type === "customer" ? "Müşteri profili için hızlı temas aksiyonları." : "Lead için güvenli temas aksiyonları."}</p></div>
    ...
    <div className="mt-3 flex flex-wrap gap-2">
      {phone && <a target="_blank" rel="noreferrer" href={`https://wa.me/${phone}?text=${encodeURIComponent(message)}`} className="rounded-full bg-[#25D366] px-4 py-2 text-sm font-black text-white">WhatsApp Gönder</a>}
      {phone && <a href={`tel:${phone}`} className="rounded-full border border-white/10 px-4 py-2 text-sm font-bold text-slate-100">Ara</a>}
      {email && <a href={`mailto:${email}?subject=${encodeURIComponent("HK Dijital")}&body=${encodeURIComponent(message)}`} className="rounded-full border border-white/10 px-4 py-2 text-sm font-bold text-slate-100">E-posta Gönder</a>}
      <button onClick={() => navigator.clipboard.writeText(message)} className="rounded-full border border-cyan-200/20 px-4 py-2 text-sm font-bold text-cyan-100">Mesajı Kopyala</button>
    </div>
    {!phone && !email && <p className="mt-3 rounded-[8px] border border-dashed border-white/10 p-3 text-xs text-slate-400">Telefon veya e-posta bilgisi bulunmadığı için otomatik bağlantı oluşturulamadı.</p>}
  </div>;
}
  ...
  return (
    <Drawer title="Başvuru Detayı" close={close}>
      ...
      <div className="mt-5 flex flex-wrap gap-2">
        ...
        <button onClick={() => askAiProvider(analyze)} disabled={analyzing || String(lead.id).startsWith("lead-")} className="inline-flex items-center gap-2 rounded-full border border-cyan-200/30 px-4 py-2 text-sm font-bold text-cyan-100 disabled:opacity-50"><Sparkles size={15} /> {analyzing ? "Analiz hazırlanıyor..." : "AI analizi oluştur"}</button>
        ...
        {whatsappUrl && <a href={whatsappUrl} target="_blank" rel="noreferrer" className="rounded-full bg-[#25D366] px-4 py-2 text-sm font-black text-white">Hızlı WhatsApp</a>}
...
function ConfirmDialog({ title, description, confirmLabel, tone = "danger", children, onCancel, onConfirm }: any) {
  ...
  return <div onMouseDown={onCancel} className="fixed inset-0 z-[80] grid place-items-center bg-black/75 p-4 backdrop-blur-sm"><div onMouseDown={(event) => event.stopPropagation()} className="admin-modal-panel w-full max-w-lg rounded-[8px] border border-white/10 bg-[#080b17] p-5 shadow-2xl"><h2 className="text-xl font-black text-white">{title}</h2><p className="mt-2 text-sm leading-6 text-slate-400">{description}</p>{children && <div className="mt-4">{children}</div>}<div className="mt-6 flex flex-wrap justify-end gap-2"><button onClick={onCancel} className="rounded-full border border-white/10 px-4 py-2 text-sm">Vazgeç</button><button onClick={onConfirm} className={`rounded-full px-5 py-2 text-sm font-black ${danger ? "bg-red-500 text-white" : "bg-amber-300 text-slate-950"}`}>{confirmLabel}</button></div></div></div>;
}
...
function Drawer({ title, close, children }: any) {
  ...
  return (
    <div onMouseDown={close} className="fixed inset-0 z-50 flex justify-end bg-black/70 backdrop-blur-sm">
      <div onMouseDown={(event) => event.stopPropagation()} className="admin-drawer-panel h-full w-full max-w-4xl overflow-auto border-l border-white/10 bg-[#080b17] p-5 shadow-2xl sm:p-7">
...
function AiAssistant({ content, setContent }: any) {
  ...
  return <Panel title="AI Studio"><div className="grid gap-5 xl:grid-cols-[240px_minmax(0,1fr)_300px]"><aside className="rounded-[8px] border border-white/10 bg-white/[0.035] p-4"><p className="text-xs font-black uppercase tracking-[.14em] text-purple-200">Prompt Library</p><div className="mt-4 grid gap-2">{shortcuts.map((item) => <button key={item} onClick={() => setPrompt(item)} className="rounded-[8px] border border-white/10 bg-black/10 p-3 text-left text-xs font-bold leading-5 text-slate-300 hover:border-purple-200/30 hover:text-purple-100">{item}</button>)}</div></aside><section className="rounded-[8px] border border-white/10 bg-white/[0.04] p-5"><div className="mb-4 rounded-[8px] border border-cyan-200/20 bg-cyan-200/10 p-3"><p className="text-sm font-black text-cyan-50">ChatGPT + Claude tarzı çalışma alanı</p><AiUsageBadge meta={activeMeta} /></div><TextArea label="Komut" value={prompt} onChange={setPrompt} /><button onClick={() => askAiProvider(generate)} className="mt-4 inline-flex w-fit items-center gap-2 rounded-full bg-cyan-300 px-5 py-3 text-sm font-black text-slate-950"><Sparkles size={17} /> AI çıktı üret</button>{message && <p className="mt-4 rounded-[8px] border border-cyan-200/20 bg-cyan-200/10 p-3 text-sm text-cyan-100">{message}</p>}{output && <div className="mt-4 rounded-[8px] border border-white/10 bg-black/30 p-4"><pre className="whitespace-pre-wrap text-sm leading-7 text-slate-200">{output}</pre><div className="mt-4 flex flex-wrap gap-2"><button onClick={() => navigator.clipboard.writeText(output)} className="inline-flex gap-2 rounded-full border border-white/10 px-4 py-2 text-sm"><Copy size={16} /> Kopyala</button><button onClick={() => setContent({ ...content, pages: { ...content.pages, home: { ...content.pages.home, subheadline: output } } })} className="rounded-full border border-white/10 px-4 py-2 text-sm">Ana sayfa alt metnine ekle</button></div></div>}{chooserModal}</section><aside className="rounded-[8px] border border-white/10 bg-white/[0.035] p-4"><p className="text-xs font-black uppercase tracking-[.14em] text-indigo-200">Output Metadata</p><div className="mt-4 grid gap-3 text-sm"><InfoItem label="Provider" value={meta.provider || activeMeta.provider} /><InfoItem label="Model" value={meta.model || activeMeta.model} /><InfoItem label="Mod" value={meta.mode || activeMeta.mode} /><InfoItem label="Durum" value={output ? "Yanıt üretildi" : message || "Bekliyor"} /></div></aside></div></Panel>;
}
...
function MetaAdsConnectionCenter({ content, setContent, api, updateApi }: any) {
  ...
  return (
    <div className="mb-5 rounded-[18px] border border-cyan-200/20 bg-slate-950/35 p-4">
      <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="text-xs font-black uppercase tracking-[.16em] text-cyan-200">Meta Reklam Bağlantıları</p>
          <h3 className="mt-1 text-xl font-black text-white">Meta Business Manager veri merkezi</h3>
          <p className="mt-1 text-sm leading-6 text-slate-300">Token değerleri sunucu tarafında kalır; bu ekranda sadece maskeli durum ve senkronizasyon sonuçları görünür.</p>
        </div>
        <span className="rounded-full border border-white/10 px-3 py-2 text-xs text-slate-300">Token: {status?.maskedToken || maskSecret(api.meta_access_token || "")}</span>
      </div>
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        ...
        <button disabled={Boolean(loading)} onClick={() => getAction("adaccounts")} className="rounded-full border border-cyan-200/25 px-4 py-2 text-xs font-black text-cyan-100 disabled:opacity-60">Reklam Hesaplarını Getir</button>
        <button disabled={Boolean(loading)} onClick={() => getAction("pages")} className="rounded-full border border-cyan-200/25 px-4 py-2 text-xs font-black text-cyan-100 disabled:opacity-60">Sayfaları Getir</button>
        <button disabled={Boolean(loading)} onClick={() => getAction("instagram")} className="rounded-full border border-cyan-200/25 px-4 py-2 text-xs font-black text-cyan-100 disabled:opacity-60">Instagram Hesaplarını Getir</button>
        <button disabled={Boolean(loading) || !form.adAccountId} onClick={() => sync("sync")} className="rounded-full border border-emerald-300/30 px-4 py-2 text-xs font-black text-emerald-100 disabled:opacity-60">{loading === "sync" ? "Senkronize ediliyor..." : "Manuel Senkronizasyon"}</button>
        <button disabled={Boolean(loading) || !form.adAccountId || !form.companyId} onClick={() => sync("report")} className="rounded-full border border-amber-300/30 px-4 py-2 text-xs font-black text-amber-100 disabled:opacity-60">Meta Verilerinden Rapor Oluştur</button>
      </div>
...
function MetaAssetList({ title, items, onPick }: any) {
  return <div className="rounded-[12px] border border-white/10 bg-black/15 p-3"><p className="text-xs font-black uppercase tracking-[.14em] text-slate-300">{title}</p><div className="mt-3 grid gap-2">{items.slice(0, 8).map((item: any) => <button key={item.id} onClick={() => onPick(item)} className="rounded-[8px] border border-white/10 p-2 text-left text-xs text-slate-300 transition hover:border-cyan-200/30 hover:bg-cyan-300/10"><strong className="block text-white">{item.name || item.username || item.id}</strong><span>{item.id}</span></button>)}{!items.length && <p className="text-xs text-slate-500">Henüz veri yok.</p>}</div></div>;
}
              ...
              <div key={company.id} className="rounded-[8px] border border-white/10 bg-black/20 p-4">
                <div className="flex flex-wrap justify-between gap-3">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    {canManageCustomers && <button onClick={() => setEditingCompanyId(editing ? "" : company.id)} className="rounded-full border border-white/10 px-3 py-2 text-xs">Düzenle</button>}
                    {canManageCustomers && <button onClick={() => archiveCompany(company)} className="rounded-full border border-amber-300/30 px-3 py-2 text-xs text-amber-100">Pasifleştir</button>}
        ...
        <div className="md:col-span-2 rounded-[8px] border border-cyan-200/20 bg-cyan-200/[0.08] p-4">
          <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <h3 className="mt-2 text-xl font-black text-white">{relatedLead ? pipelineStageForLead(relatedLead) : "Satış hunisine bağlı değil"}</h3>
              <p className="mt-1 text-sm leading-6 text-cyan-50/80">Kampanya, teklif, görev, ödeme ve rapor akışı bu müşteri profili üzerinden takip edilir.</p>
            </div>
          ...
          {relatedLead ? (
            ...
            <div className="rounded-[8px] border border-dashed border-white/10 p-4">
              <p className="text-sm leading-6 text-slate-300">Bu müşteri henüz satış hunisine bağlanmamış.</p>
  ...
  return (
    <div className="grid gap-5">
      ...
      <div className="rounded-[14px] border border-white/10 bg-black/15 p-4">
        <h3 className="font-black text-white">Meta Kampanyaları</h3>
        <div className="mt-4 grid gap-3">
          {summaries.map((item: any) => <div key={item.campaignId} className="grid gap-3 rounded-[10px] border border-white/10 p-3 md:grid-cols-[1fr_.5fr_.5fr_.5fr_.5fr_auto] md:items-center">
            <span><strong className="block text-white">{item.campaignName}</strong><small className="text-slate-400">{item.status}</small></span>
            <span className="text-sm text-slate-300">{Number(item.spend || 0).toLocaleString("tr-TR")} TL</span>
            <span className="text-sm text-slate-300">{Number(item.results || 0).toLocaleString("tr-TR")} sonuç</span>
            <span className="text-sm text-slate-300">{Number(item.ctr || 0).toFixed(2)}% CTR</span>
            <span className="text-sm text-slate-300">{Number(item.cpc || 0).toFixed(2)} CPC</span>
            <span className="flex flex-wrap gap-2"><button onClick={() => pull("report")} className="rounded-full border border-amber-300/30 px-3 py-1.5 text-xs text-amber-100">Rapor Oluştur</button><button onClick={() => recordActionDetail("Meta Kampanya Detayı", [["Kampanya", item.campaignName], ["Harcama", `${Number(item.spend || 0).toLocaleString("tr-TR")} TL`], ["Sonuçlar", item.results], ["CTR", `${Number(item.ctr || 0).toFixed(2)}%`], ["CPC", Number(item.cpc || 0).toFixed(2)]])} className="rounded-full border border-cyan-200/25 px-3 py-1.5 text-xs text-cyan-100">Detay Gör</button><button className="rounded-full border border-white/10 px-3 py-1.5 text-xs text-slate-200">Kampanyayı Eşleştir</button></span>
          </div>)}
          {!summaries.length && <p className="rounded-[8px] border border-dashed border-white/10 p-5 text-sm text-slate-400">Henüz Meta kampanya verisi yok. “Verileri Çek” ile senkronizasyon başlatın.</p>}
        </div>
      </div>
    </div>
  );
...
function CustomerReportsEditor({ company, content, setContent, save, items, notify, canManage = true }: any) {
  ...
  return <div><div className="mb-4 flex flex-wrap items-center justify-between gap-3"><div><h3 className="font-black text-white">Raporlar</h3><p className="mt-1 text-sm text-slate-400">Raporlar, müşteri paneli görünürlüğü ve PDF/Yazdır akışıyla birlikte yönetilir.</p></div>{canManage && <button onClick={add} className="rounded-full bg-cyan-300 px-4 py-2 text-xs font-black text-slate-950">Rapor Oluştur</button>}</div><div className="grid gap-3">{visibleItems.map((item) => <div key={item.id} className={`rounded-[8px] border p-4 ${isArchivedRecord(item) || item.archived ? "border-amber-300/25 bg-amber-300/[0.06]" : "border-white/10 bg-black/20"}`}><div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3"><SelectField label="Rapor türü" value={item.report_type || "Genel Dijital Performans Raporu"} onChange={(value) => update(item.id, { report_type: value })} options={["Meta Reklam Raporu", "Google Ads Raporu", "Sosyal Medya Yönetimi Raporu", "Genel Dijital Performans Raporu"]} /><Field label="Dönem" value={item.period || ""} onChange={(value) => update(item.id, { period: value })} /><SelectField label="Durum" value={item.status || "Taslak"} onChange={(value) => update(item.id, { status: value })} options={["Taslak", "Hazır", "Yayınlandı", "Arşivlendi"]} /><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={Boolean(item.visible_to_customer)} onChange={(event) => update(item.id, { visible_to_customer: event.target.checked })} /> {item.visible_to_customer ? "Müşteriye Göster" : "Müşteriden Gizle"}</label><InfoItem label="Oluşturulma tarihi" value={formatDateTime(item.created_at)} /><InfoItem label="Güncellenme tarihi" value={formatDateTime(item.updated_at)} /><div className="md:col-span-2 xl:col-span-3"><TextArea label="Rapor özeti" value={item.summary || item.customer_note || ""} onChange={(value) => update(item.id, { summary: value, customer_note: value })} /></div><div className="md:col-span-2 xl:col-span-3"><TextArea label="AI yorum / sonraki öneri" value={item.ai_interpretation || ""} onChange={(value) => update(item.id, { ai_interpretation: value })} /></div></div><div className="mt-4 flex flex-wrap justify-end gap-2"><button onClick={() => aiInterpret(item)} className="rounded-full border border-cyan-200/25 px-4 py-2 text-xs text-cyan-100">AI Yorumla</button><button onClick={() => window.print()} className="rounded-full border border-white/10 px-4 py-2 text-xs text-slate-100">PDF / Yazdır</button>{isArchivedRecord(item) || item.archived ? <button onClick={() => update(item.id, { archived_at: null, deleted_at: null, archived: false, status: "Taslak" })} className="rounded-full border border-amber-300/30 px-4 py-2 text-xs text-amber-100">Arşivden Çıkar</button> : <button onClick={() => update(item.id, { archived_at: new Date().toISOString(), archived: true, status: "Arşivlendi" })} className="rounded-full border border-amber-300/30 px-4 py-2 text-xs text-amber-100">Arşivle</button>}<button onClick={() => confirm("Bu raporu silmek istediğinize emin misiniz?") && update(item.id, { deleted_at: new Date().toISOString(), status: "Silindi" })} className="rounded-full border border-red-300/30 px-4 py-2 text-xs text-red-200">Sil</button><button onClick={() => run(`report-${item.id}`, "Kaydediliyor...", "Kaydedildi", () => save?.())} className="rounded-full bg-cyan-300 px-4 py-2 text-xs font-black text-slate-950">{label(`report-${item.id}`, "Kaydet")}</button><button onClick={() => window.location.reload()} className="rounded-full border border-white/10 px-4 py-2 text-xs text-slate-200">Vazgeç</button></div></div>)}{!visibleItems.length && <p className="rounded-[8px] border border-dashed border-white/10 p-5 text-sm text-slate-400">Bu müşteri için kanal bazlı rapor yok.</p>}</div></div>;
}
...
function CustomerFilesEditor({ company, content, setContent, save, items, notify, canManage = true }: any) {
  ...
  const add = () => updateCollection(content, setContent, "customerFiles", [{ id: createLocalId(), company_id: company.id, title: "Yeni Dosya", file_type: "Diğer", description: "", file_url: "", visible_to_customer: false, status: "Aktif", created_at: new Date().toISOString(), updated_at: new Date().toISOString() }, ...allItems]);
  return <div><div className="mb-4 flex flex-wrap items-center justify-between gap-3"><div><h3 className="font-black text-white">Dosyalar</h3><p className="mt-1 text-sm text-slate-400">Müşteri panelinde sadece görünür olarak işaretlenen dosyalar gösterilir.</p></div>{canManage && <button onClick={add} className="rounded-full bg-cyan-300 px-4 py-2 text-xs font-black text-slate-950">Dosya Ekle</button>}</div><div className="grid gap-3">{visibleItems.map((item) => <div key={item.id} className={`rounded-[8px] border p-4 ${isArchivedRecord(item) ? "border-amber-300/25 bg-amber-300/[0.06]" : "border-white/10 bg-black/20"}`}><div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3"><Field label="Dosya başlığı" value={item.title || ""} onChange={(value) => update(item.id, { title: value })} /><SelectField label="Dosya türü" value={item.file_type || "Diğer"} onChange={(value) => update(item.id, { file_type: value })} options={["Teklif", "Sözleşme", "Fatura", "Rapor", "Görsel", "Diğer"]} /><Field label="Dosya URL" value={item.file_url || ""} onChange={(value) => update(item.id, { file_url: value })} /><Upload onUrl={(url) => update(item.id, { file_url: url })} /><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={Boolean(item.visible_to_customer)} onChange={(event) => update(item.id, { visible_to_customer: event.target.checked })} /> {item.visible_to_customer ? "Müşteriye Göster" : "Sadece Yönetici"}</label><InfoItem label="Güncellenme tarihi" value={formatDateTime(item.updated_at)} /><div className="md:col-span-2 xl:col-span-3"><TextArea label="Açıklama" value={item.description || ""} onChange={(value) => update(item.id, { description: value })} /></div></div><div className="mt-4 flex flex-wrap justify-end gap-2">{isArchivedRecord(item) ? <button onClick={() => update(item.id, { archived_at: null, deleted_at: null, status: "Aktif" })} className="rounded-full border border-amber-300/30 px-4 py-2 text-xs text-amber-100">Arşivden Çıkar</button> : <button onClick={() => update(item.id, { archived_at: new Date().toISOString(), status: "Arşivlendi" })} className="rounded-full border border-amber-300/30 px-4 py-2 text-xs text-amber-100">Arşivle</button>}<button onClick={() => confirm("Bu dosyayı silmek istediğinize emin misiniz?") && update(item.id, { deleted_at: new Date().toISOString(), status: "Silindi" })} className="rounded-full border border-red-300/30 px-4 py-2 text-xs text-red-200">Sil</button><button onClick={() => run(`file-${item.id}`, "Kaydediliyor...", "Kaydedildi", () => save?.())} className="rounded-full bg-cyan-300 px-4 py-2 text-xs font-black text-slate-950">{label(`file-${item.id}`, "Kaydet")}</button><button onClick={() => window.location.reload()} className="rounded-full border border-white/10 px-4 py-2 text-xs text-slate-200">Vazgeç</button></div></div>)}{!visibleItems.length && <p className="rounded-[8px] border border-dashed border-white/10 p-5 text-sm text-slate-400">Bu müşteri için dosya yok.</p>}</div></div>;
}
          ...
          return (
            <div key={campaign.id} className={`rounded-[8px] border p-4 ${archived ? "border-amber-300/25 bg-amber-300/[0.06]" : "border-white/10 bg-black/20"}`}>
              ...
              <div className="mt-4 flex flex-wrap justify-end gap-2">
                {canManage && <button onClick={() => createReport(campaign)} className="rounded-full border border-emerald-300/30 px-4 py-2 text-xs text-emerald-100">Rapor taslağı oluştur</button>}
                {canManage && <button onClick={() => copy(campaign)} className="rounded-full border border-white/10 px-4 py-2 text-xs text-slate-200">Kopyala</button>}
                {canManage && (archived ? <button onClick={() => update(campaign.id, { archived_at: null, deleted_at: null, status: "Planlandı" })} className="rounded-full border border-amber-300/30 px-4 py-2 text-xs text-amber-100">Arşivden Çıkar</button> : <button onClick={() => archive(campaign)} className="rounded-full border border-red-300/30 px-4 py-2 text-xs text-red-200">Sil / Arşivle</button>)}
...
function CustomerTasksEditor({ company, content, setContent, save, items, notify, canManage = true }: any) {
  ...
  return <div><div className="mb-4 flex flex-wrap items-center justify-between gap-3"><div><h3 className="font-black text-white">Yapılacaklar</h3><p className="mt-1 text-sm text-slate-400">Bu görevler Görevler modülü ve Dashboard operasyon özetleriyle eş zamanlıdır.</p></div>{canManage && <button onClick={add} className="rounded-full bg-cyan-300 px-4 py-2 text-xs font-black text-slate-950">Görev Ekle</button>}</div><div className="mb-4 grid gap-3 md:grid-cols-4"><SelectField label="Durum filtresi" value={statusFilter} onChange={setStatusFilter} options={taskHistoryFilters} /><Field label="Başlangıç tarihi" type="date" value={startDate} onChange={setStartDate} /><Field label="Bitiş tarihi" type="date" value={endDate} onChange={setEndDate} /><button onClick={() => { setStatusFilter("Tümü"); setStartDate(""); setEndDate(""); }} className="self-end rounded-[8px] border border-white/10 px-4 py-3 text-sm font-black text-slate-100">Filtreleri Temizle</button></div><div className="grid gap-3">{visibleItems.map((item) => <div key={item.id} className={`rounded-[8px] border p-4 ${isArchivedRecord(item) ? "border-amber-300/25 bg-amber-300/[0.06]" : "border-white/10 bg-black/20"}`}><div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3"><Field label="Başlık" value={item.title || ""} onChange={(value) => update(item.id, { title: value })} /><SelectField label="Durum" value={item.status || "Yapılacak"} onChange={(value) => canManage && setStatus(item.id, value)} options={taskStatusOptions} /><SelectField label="Öncelik" value={item.priority || "Orta"} onChange={(value) => update(item.id, { priority: value })} options={["Düşük", "Orta", "Yüksek", "Kritik"]} /><Field label="Son tarih" type="date" value={item.due_date || ""} onChange={(value) => update(item.id, { due_date: value })} /><SelectField label="Atanan kullanıcı" value={item.assigned_user_id || ""} onChange={(value) => update(item.id, { assigned_user_id: value })} options={(content.users || []).map((user) => ({ value: user.id, label: user.full_name || user.email }))} placeholder="Atanmadı" /><InfoItem label="Tamamlanma tarihi" value={formatDateTime(item.completed_at)} /><InfoItem label="Oluşturulma tarihi" value={formatDateTime(item.created_at)} /><InfoItem label="Güncellenme tarihi" value={formatDateTime(item.updated_at)} /><div className="md:col-span-2 xl:col-span-3"><TextArea label="Açıklama / not" value={item.description || item.notes || ""} onChange={(value) => update(item.id, { description: value, notes: value })} /></div></div><div className="mt-4 flex flex-wrap justify-end gap-2">{canManage && (isArchivedRecord(item) ? <button onClick={() => restore(item.id)} className="rounded-full border border-amber-300/30 px-4 py-2 text-xs text-amber-100">Arşivden Çıkar</button> : <button onClick={() => archive(item.id)} className="rounded-full border border-amber-300/30 px-4 py-2 text-xs text-amber-100">Arşivle</button>)}{canManage && <button onClick={() => setStatus(item.id, item.status === "Tamamlandı" ? "Yapılacak" : "Tamamlandı")} className="rounded-full border border-emerald-300/30 px-4 py-2 text-xs text-emerald-100">{item.status === "Tamamlandı" ? "Tekrar Aç" : "Tamamlandı Yap"}</button>}{canManage && <button onClick={() => run(`task-${item.id}`, "Kaydediliyor...", "Kaydedildi", () => save?.())} className="rounded-full bg-cyan-300 px-4 py-2 text-xs font-black text-slate-950">{label(`task-${item.id}`, "Kaydet")}</button>}<button onClick={() => window.location.reload()} className="rounded-full border border-white/10 px-4 py-2 text-xs text-slate-200">Vazgeç</button></div></div>)}{!visibleItems.length && <p className="rounded-[8px] border border-dashed border-white/10 p-5 text-sm text-slate-400">Bu müşteri için görev kaydı bulunamadı.</p>}</div></div>;
}
...
function ActivityLogs({ content, setContent }: any) {
  ...
  return (
    <Panel title="Aktivite Akışı">
      ...
      <div className="mb-4 flex flex-wrap gap-2">
        <button disabled={!selectedIds.length} onClick={() => bulk({ is_seen: true, status: "Görüldü" })} className="rounded-full border border-emerald-300/30 px-3 py-2 text-xs text-emerald-100 disabled:opacity-40">Toplu Görüldü Yap</button>
        <button disabled={!selectedIds.length} onClick={() => bulk({ archived_at: new Date().toISOString(), status: "Arşivlendi" })} className="rounded-full border border-amber-300/30 px-3 py-2 text-xs text-amber-100 disabled:opacity-40">Toplu Arşivle</button>
...
function ActivityList({ items, empty, selectedIds = [], toggleSelected, updateLog, openDetail, undoLog }: any) {
  return <div className="grid gap-3">{items.map((item) => {
    ...
    return <div key={item.id} className={`rounded-[8px] border p-4 ${item.archived_at ? "border-amber-300/20 bg-amber-300/5" : "border-white/10 bg-black/20"}`}><div className="flex flex-wrap items-start justify-between gap-3"><div className="flex min-w-0 gap-3">{toggleSelected && <input type="checkbox" checked={selectedIds.includes(item.id)} onChange={() => toggleSelected(item.id)} className="mt-1" />}<div><p className="font-black text-white">{item.details?.message || `${item.entity} · ${item.action}`}</p><p className="mt-1 text-sm text-slate-400">{item.actor_name || "Sistem"} · {roleOptions.find((role) => role.value === item.role)?.label || item.role || "Sistem"} · {item.module || item.entity}</p><p className="mt-3 text-xs text-slate-500">{formatDateTime(item.created_at)}</p></div></div><div className="flex flex-wrap justify-end gap-2"><span className="rounded-full bg-cyan-200/10 px-3 py-1 text-xs font-bold text-cyan-100">{item.action_type || item.action}</span><span className="rounded-full bg-white/10 px-3 py-1 text-xs font-bold text-slate-200">{status}</span>{item.is_critical && <span className="rounded-full bg-red-300/15 px-3 py-1 text-xs font-bold text-red-100">Kritik</span>}</div></div>{updateLog && <div className="mt-4 flex flex-wrap gap-2"><button onClick={() => updateLog(item.id, { is_seen: true, status: "Görüldü" })} className="rounded-full border border-emerald-300/30 px-3 py-2 text-xs text-emerald-100">Görüldü Yap</button><button onClick={() => updateLog(item.id, { is_seen: false, status: "Görülmedi" })} className="rounded-full border border-white/10 px-3 py-2 text-xs text-slate-200">Görülmedi Yap</button><button onClick={() => updateLog(item.id, { archived_at: new Date().toISOString(), status: "Arşivlendi" })} className="rounded-full border border-amber-300/30 px-3 py-2 text-xs text-amber-100">Arşivle</button><button onClick={() => confirm("Bu log kaydını silmek istediğinize emin misiniz?") && updateLog(item.id, { deleted_at: new Date().toISOString(), status: "Silindi" })} className="rounded-full border border-red-300/30 px-3 py-2 text-xs text-red-200">Sil</button>{canUndo && <button onClick={() => undoLog?.(item)} className="rounded-full border border-cyan-300/30 px-3 py-2 text-xs text-cyan-100">Geri Al</button>}<button onClick={() => openDetail?.(item)} className="ml-auto rounded-full bg-cyan-300 px-3 py-2 text-xs font-black text-slate-950">Detay</button></div>}</div>;
  })}{!items.length && <p className="text-sm text-slate-400">{empty}</p>}</div>;
}
          ...
          return (
            <div key={campaign.id} className={`rounded-[8px] border p-4 ${archived ? "border-amber-300/25 bg-amber-300/[0.06]" : "border-white/10 bg-black/15"}`}>
              ...
              <div className="mt-4 flex flex-wrap justify-end gap-2">
                ...
                {canManage && <button onClick={() => copyCampaign(campaign)} className="rounded-full border border-white/10 px-4 py-2 text-xs text-slate-200">Kopyala</button>}
                {canManage && (archived ? <button onClick={() => update(campaign.id, { archived_at: null, deleted_at: null, status: "Planlandı" }, "Kampanya arşivden çıkarıldı")} className="rounded-full border border-amber-300/30 px-4 py-2 text-xs text-amber-100">Arşivden Çıkar</button> : <button onClick={() => archiveCampaign(campaign)} className="rounded-full border border-red-300/30 px-4 py-2 text-xs text-red-200">Sil / Arşivle</button>)}
              </div>
            </div>
          );
        })}
...
function MetaAnalysisSection() {
  ...
  return <Panel title="Meta Analiz"><div className="mb-5 rounded-[8px] border border-orange-300/20 bg-gradient-to-br from-orange-300/12 via-yellow-200/5 to-transparent p-5"><div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-xs font-black uppercase tracking-[.16em] text-orange-200">Meta Business Suite Workspace</p><h2 className="mt-2 text-2xl font-black text-white">Facebook ve Instagram reklam sinyallerini ayrı analiz edin</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-slate-400">İl, ilçe ve sektör seçimine göre Meta Ad Library odaklı gözlemler, CTA ve kreatif metin özetleri üretir.</p></div><div className="flex flex-wrap gap-2"><button disabled={loading} onClick={() => askAiProvider(analyze)} className="rounded-[8px] bg-orange-300 px-4 py-3 text-xs font-black text-slate-950 disabled:opacity-60">Meta Sonuçlarını Yenile</button><button onClick={testMetaConnection} className="rounded-[8px] border border-orange-200/30 px-4 py-3 text-xs font-black text-orange-100">Meta Bağlantısını Test Et</button></div></div></div><div className="mb-5 grid gap-3 md:grid-cols-3"><div className="rounded-[8px] border border-orange-200/20 bg-orange-300/[0.08] p-4"><p className="text-xs font-black text-orange-100">Ad Preview</p><p className="mt-2 text-sm leading-6 text-slate-300">Kreatif, CTA ve platform sinyalleri kart bazında okunur.</p></div><div className="rounded-[8px] border border-cyan-200/20 bg-cyan-300/[0.08] p-4"><p className="text-xs font-black text-cyan-100">CTA Analysis</p><p className="mt-2 text-sm leading-6 text-slate-300">Mesaj, form, trafik ve teklif açısı birlikte değerlendirilir.</p></div><div className="rounded-[8px] border border-pink-200/20 bg-pink-300/[0.08] p-4"><p className="text-xs font-black text-pink-100">Creative Analysis</p><p className="mt-2 text-sm leading-6 text-slate-300">Reklam dili ve kreatif fırsatlar CRM'e taşınabilir.</p></div></div><AnalysisSearchForm form={form} setForm={setForm} loading={loading} onSubmit={() => askAiProvider(analyze)} buttonLabel="Meta Reklamlarını Analiz Et" />{metaStatusMessage && <p className="mt-4 rounded-[8px] border border-orange-300/25 bg-orange-300/10 p-3 text-sm text-orange-100">{metaStatusMessage}</p>}{warning && <p className="mt-4 rounded-[8px] border border-amber-300/25 bg-amber-300/10 p-3 text-sm text-amber-100">{warning}</p>}{error && <p className="mt-4 rounded-[8px] border border-red-300/25 bg-red-500/10 p-3 text-sm text-red-100">{error}</p>}<div className="mt-5 grid gap-4 lg:grid-cols-2">{results.map((item) => { const id = item.id || item.name; return <AnalysisResultCard key={id} kind="meta" item={item} form={form} aiMeta={aiMeta} saved={crm.savedIds[id]} saving={crm.savingId === id} message={crm.messages[id]} onOpen={() => askAiProvider(() => crm.setSelected(item))} onSave={() => crm.save(item)} />; })}{!loading && !error && !results.length && <p className="rounded-[8px] border border-dashed border-white/10 p-6 text-center text-sm text-slate-400 lg:col-span-2">Bu seçim için sonuç bulunamadı.</p>}</div>{crm.selected && <AnalysisDetailModal kind="meta" item={crm.selected} form={form} aiMeta={aiMeta} saved={crm.savedIds[crm.selected.id || crm.selected.name]} saving={crm.savingId === (crm.selected.id || crm.selected.name)} message={crm.messages[crm.selected.id || crm.selected.name]} onClose={() => crm.setSelected(null)} onSave={() => crm.save(crm.selected)} />}{chooserModal}</Panel>;
}
...
function GoogleAdsAnalysisSection() {
  ...
  return <Panel title="Google Ads Analiz"><div className="mb-5 rounded-[8px] border border-yellow-300/20 bg-gradient-to-br from-yellow-300/12 via-orange-200/5 to-transparent p-5"><p className="text-xs font-black uppercase tracking-[.16em] text-yellow-200">Google Ads Workspace</p><h2 className="mt-2 text-2xl font-black text-white">Arama görünürlüğü ve yerel reklam fırsatlarını ayrı analiz edin</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-slate-400">Google Maps ve işletme sinyallerinden arama görünürlüğü, anahtar kelime fırsatı ve kampanya tipi önerileri üretir.</p></div><div className="mb-5 grid gap-3 md:grid-cols-4"><div className="rounded-[8px] border border-sky-200/20 bg-sky-300/[0.08] p-4"><p className="text-xs font-black text-sky-100">Keyword Opportunities</p><p className="mt-2 text-sm leading-6 text-slate-300">Yüksek niyetli arama kelimeleri.</p></div><div className="rounded-[8px] border border-emerald-200/20 bg-emerald-300/[0.08] p-4"><p className="text-xs font-black text-emerald-100">Business Signals</p><p className="mt-2 text-sm leading-6 text-slate-300">Maps, yorum ve görünürlük sinyalleri.</p></div><div className="rounded-[8px] border border-amber-200/20 bg-amber-300/[0.08] p-4"><p className="text-xs font-black text-amber-100">Competition Level</p><p className="mt-2 text-sm leading-6 text-slate-300">Rekabet ve bütçe önerileri.</p></div><div className="rounded-[8px] border border-indigo-200/20 bg-indigo-300/[0.08] p-4"><p className="text-xs font-black text-indigo-100">Campaign Cards</p><p className="mt-2 text-sm leading-6 text-slate-300">Arama, lokal ve remarketing açısı.</p></div></div><AnalysisSearchForm form={form} setForm={setForm} loading={loading} onSubmit={() => askAiProvider(analyze)} buttonLabel="Google Reklamlarını Analiz Et" />{warning && <p className="mt-4 rounded-[8px] border border-amber-300/25 bg-amber-300/10 p-3 text-sm text-amber-100">{warning}</p>}{error && <p className="mt-4 rounded-[8px] border border-red-300/25 bg-red-500/10 p-3 text-sm text-red-100">{error}</p>}<div className="mt-5 grid gap-4 lg:grid-cols-2">{results.map((item) => { const id = item.id || item.name; return <AnalysisResultCard key={id} kind="google" item={item} form={form} aiMeta={aiMeta} saved={crm.savedIds[id]} saving={crm.savingId === id} message={crm.messages[id]} onOpen={() => askAiProvider(() => crm.setSelected(item))} onSave={() => crm.save(item)} />; })}{!loading && !error && !results.length && <p className="rounded-[8px] border border-dashed border-white/10 p-6 text-center text-sm text-slate-400 lg:col-span-2">Bu seçim için sonuç bulunamadı.</p>}</div>{crm.selected && <AnalysisDetailModal kind="google" item={crm.selected} form={form} aiMeta={aiMeta} saved={crm.savedIds[crm.selected.id || crm.selected.name]} saving={crm.savingId === (crm.selected.id || crm.selected.name)} message={crm.messages[crm.selected.id || crm.selected.name]} onClose={() => crm.setSelected(null)} onSave={() => crm.save(crm.selected)} />}{chooserModal}</Panel>;
}
    ...
    <Panel title="Sosyal İstihbarat Merkezi">
      <div className="mb-7 overflow-hidden rounded-[8px] border border-yellow-300/25 bg-gradient-to-br from-yellow-300/16 via-cyan-300/8 to-white/[0.03] p-6 shadow-[0_26px_90px_rgba(250,204,21,.10)]">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <h2 className="mt-2 text-3xl font-black text-white">Sosyal profilleri premium bir audit akışında inceleyin</h2>
            <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-300">İşletme bilgileri, profil alanları, ekran görüntüleri ve seçili aksiyonlar daha temiz bir akışta birlikte değerlendirilir.</p>
          </div>
          <div className="w-full max-w-xs rounded-[8px] border border-white/10 bg-black/20 p-3">
      ...
      <div className="grid gap-7 xl:grid-cols-2 xl:items-start">
        <div className="grid gap-5">
          ...
          <section className="grid gap-4">
            <div><p className="text-xs font-black uppercase tracking-[.14em] text-yellow-200">Platform kartları</p><h3 className="mt-1 text-lg font-black text-white">Profil kaynakları</h3></div>
            <div className="grid gap-4 lg:grid-cols-2">
              {platforms.map((item, index) => {
                const status = profileFetchStatus(item);
                return <div key={item.platform} className="rounded-[8px] border border-white/10 bg-white/[0.04] p-5 shadow-[0_16px_50px_rgba(15,23,42,.18)]"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-base font-black text-white">{item.platform}</p><p className="mt-1 text-xs leading-5 text-slate-400">{item.platform === "Instagram" ? "Kullanıcı adı girildiğinde profil URL otomatik oluşur." : "Profil URL veya kullanıcı adı ekleyin."}</p></div><span className={"rounded-full px-3 py-1 text-xs font-black " + (status === "Profil bilgileri alındı" ? "bg-emerald-300/12 text-emerald-100" : status === "Sınırlı veri" ? "bg-amber-300/12 text-amber-100" : "bg-slate-300/10 text-slate-300")}>{status}</span></div><div className="mt-4 grid gap-4"><Field label={usernameLabel(item.platform)} value={item.username} onChange={(username) => updatePlatform(index, { username: String(username || "").replace(/^@+/, "") })} /><Field label={urlLabel(item.platform)} value={item.profileUrl} onChange={(profileUrl) => updatePlatform(index, { profileUrl })} /><Field label="Profil görsel URL (opsiyonel)" value={item.profileImageUrl} onChange={(profileImageUrl) => updatePlatform(index, { profileImageUrl, fetchStatus: profileImageUrl ? "Sınırlı veri" : item.fetchStatus })} /></div><div className="mt-4 flex flex-wrap gap-2">{item.platform === "Instagram" && <button type="button" disabled={profileFetching[item.platform]} onClick={() => scanProfile(index)} className="rounded-full bg-yellow-300 px-4 py-2 text-xs font-black text-slate-950 disabled:opacity-60">{profileFetching[item.platform] ? "Taranıyor..." : "Instagram Profilini Tara"}</button>}{item.platform !== "Instagram" && <button type="button" disabled={profileFetching[item.platform]} onClick={() => scanProfile(index)} className="rounded-full border border-yellow-200/30 px-4 py-2 text-xs font-black text-yellow-100 disabled:opacity-60">{profileFetching[item.platform] ? "Taranıyor..." : "Profili Tara"}</button>}{item.profileUrl && <a href={item.profileUrl} target="_blank" rel="noreferrer" className="rounded-full border border-cyan-200/20 px-4 py-2 text-xs font-black text-cyan-100">Profili Aç</a>}</div>{(item.displayName || item.bio || item.fetchWarning) && <div className="mt-4 rounded-[8px] border border-dashed border-white/10 bg-black/15 p-3 text-xs leading-5 text-slate-300"><p className="font-black text-white">{item.displayName || item.publicTitle || item.username || item.platform}</p>{item.bio && <p className="mt-1">{item.bio}</p>}{item.website && <p className="mt-1 break-all text-emerald-100">{item.website}</p>}{item.fetchWarning && <p className="mt-2 text-amber-100">{item.fetchWarning}</p>}</div>}</div>;
              })}
            </div>
          </section>
      ...
      <section className="mt-6 rounded-[8px] border border-rose-200/20 bg-rose-300/[0.07] p-5 shadow-[0_18px_70px_rgba(0,0,0,.14)]">
        <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_300px]">
          <div>
            <p className="text-xs font-black uppercase tracking-[.16em] text-rose-200">PDF Audit Workspace</p>
            <h3 className="mt-2 text-xl font-black text-white">Doküman önizleme ve dışa aktarma alanı</h3>
            <p className="mt-2 text-sm leading-6 text-slate-300">İşletme bilgileri, profil verisi, ekran görüntüleri ve seçili analiz çıktıları PDF audit akışına hazırlanır.</p>
            <div className="mt-4 grid gap-2 sm:grid-cols-3">
              {["Profil özeti", "Audit bölümleri", "Export kontrolleri"].map((item) => <span key={item} className="rounded-[8px] border border-white/10 bg-black/10 p-3 text-xs font-black text-slate-200">{item}</span>)}
            </div>
          </div>
          <div className="rounded-[8px] border border-white/10 bg-white/[0.05] p-4">
            ...
            <p className="mt-2 text-lg font-black text-white">{form.businessName || previewName}</p>
            <p className="mt-1 text-sm text-slate-300">{form.city} / {form.district} · {form.sector}</p>
      ...
      {message && <p className="mt-4 rounded-[8px] border border-amber-300/25 bg-amber-300/10 p-3 text-sm text-amber-100">{message}</p>}
      {crmMessage && <p className="mt-4 rounded-[8px] border border-cyan-200/20 bg-cyan-200/10 p-3 text-sm text-cyan-100">{crmMessage}</p>}
      <div className="mt-6 grid gap-4">{outputs.map((item) => <div key={item.action} className="rounded-[8px] border border-white/10 bg-white/[0.045] p-5"><p className="text-xs font-black uppercase tracking-[.16em] text-yellow-200">{item.action}</p><AiUsageBadge meta={item.ai} /><pre className="mt-4 whitespace-pre-wrap text-sm leading-7 text-slate-200">{item.text}</pre></div>)}{!loading && !outputs.length && <p className="rounded-[8px] border border-dashed border-white/10 p-6 text-center text-sm text-slate-400">Sosyal istihbarat çıktısı için profil, ekran görüntüsü veya işletme bilgisi girip aksiyon seçin.</p>}</div>
      {chooserModal}
    </Panel>
  );
}
...
${notes || "Satış garantisi verilmez. Sistem; reklam bütçesini daha kontrollü yönetmeye, performansı görünür kılmaya ve lead takibini düzenlemeye yardımcı olur."}`);
  ...
  function printProposal() {
    ...
    printWindow.document.write(`<html><head><title>HK Dijital Teklif</title><style>body{font-family:Arial,sans-serif;padding:32px;line-height:1.6;color:#0f172a}pre{white-space:pre-wrap}h1{color:#0891b2}</style></head><body><h1>HK Dijital Teklif</h1><pre>${result.replace(/[<>&]/g, (char) => ({ "<": "&lt;", ">": "&gt;", "&": "&amp;" })[char] || char)}</pre></body></html>`);
