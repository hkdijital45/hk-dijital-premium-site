  ...
  return (
    <div className="fixed inset-0 z-[11000] grid place-items-center bg-slate-950/75 p-3 sm:p-6" role="presentation" onMouseDown={onClose}>
      <section
        ...
        className="flex max-h-[calc(100dvh-1.5rem)] w-full max-w-4xl flex-col overflow-hidden rounded-[18px] border border-[var(--admin-border)] bg-[var(--admin-surface)] text-[var(--admin-text-primary)] shadow-2xl sm:max-h-[calc(100dvh-3rem)]"
        onMouseDown={(event) => event.stopPropagation()}
      >
        <header className="flex shrink-0 items-start justify-between gap-4 border-b border-[var(--admin-border)] bg-[var(--admin-surface)] px-4 py-4 sm:px-6">
          ...
          <button type="button" onClick={onClose} aria-label="Rapor önizlemesini kapat" className="grid size-11 shrink-0 place-items-center rounded-full border border-[var(--admin-border)] text-[var(--admin-text-secondary)] hover:bg-[var(--admin-surface-soft)] focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-cyan-100">
            <X size={19} />
          </button>
        </header>
        ...
        <div className="premium-scrollbar min-h-0 flex-1 overflow-y-auto overflow-x-hidden px-4 py-5 sm:px-6">
            ...
            <div className="grid gap-4">
              <section className="grid gap-3 rounded-[14px] border border-cyan-200 bg-cyan-50 p-4 sm:p-5 lg:grid-cols-[minmax(0,1fr)_160px]">
                ...
                {digitalReport.score !== null && digitalReport.score !== undefined && (
                  <div className="rounded-[14px] border border-cyan-200 bg-[var(--admin-surface)] p-4 text-cyan-950">
              ...
              {digitalReport.sections.length ? (
                <div className="grid gap-4 lg:grid-cols-2">
                  {digitalReport.sections.map((section) => (
                    <article key={section.title} className="min-w-0 rounded-[14px] border border-[var(--admin-border)] bg-[var(--admin-surface-soft)] p-4 sm:p-5">
                      <h3 className="break-words text-base font-black text-[var(--admin-text-primary)]">{section.title}</h3>
                      {section.summary && <p className="mt-2 break-words text-sm leading-6 text-[var(--admin-text-secondary)]">{section.summary}</p>}
                      {section.items.length > 0 ? (
                        <ul className="mt-3 grid gap-2 text-sm leading-6 text-[var(--admin-text-secondary)]">
                        ...
                        <p className="mt-3 rounded-[10px] border border-dashed border-slate-300 bg-[var(--admin-surface)] p-3 text-sm text-[var(--admin-text-muted)]">Bu bölüm için madde bulunamadı.</p>
                      )}
                    </article>
                  ))}
                </div>
              ...
              <div className="mt-5 grid gap-4 md:grid-cols-2">
                {sections.map((section) => (
                  <article key={section.title} className="min-w-0 rounded-[14px] border border-[var(--admin-border)] bg-[var(--admin-surface-soft)] p-4">
                    ...
                    {section.summary && <p className="mt-2 text-sm leading-6 text-[var(--admin-text-secondary)]">{section.summary}</p>}
                    <ul className="mt-3 grid gap-2 text-sm leading-6 text-[var(--admin-text-secondary)]">
          ...
          {report.metadata?.provider_label && (
            <p className="mt-5 text-xs leading-5 text-[var(--admin-text-muted)]">
