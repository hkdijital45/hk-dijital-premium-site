...
export function AdsOperatingCenter({ content, setActive }: GrowthProps) {
  ...
  const dataDrivenRecommendations = [
    ...
    },
    {
      ...
      target: !reports.length ? "/hk-admin/musteri-raporlari" : "/hk-admin/teklif-takip-merkezi"
    }
...
export function FunnelBuilderCenter({ content, setActive }: GrowthProps) {
  ...
  const visibleSteps = showMissingOnly ? missing : steps;
  return (
    <div className="grid gap-5">
      <PremiumPageHeader eyebrow="Funnel Planlayıcı" title="Müşteriye özel funnel şablonu oluştur" description="Sürükle-bırak yerine güvenli kart tabanlı planlayıcı ile müşteri, amaç, kanal ve eksik adımları planlayın." actionLabel={showMissingOnly ? "Tüm Adımları Göster" : "Eksikleri Göster"} onAction={() => setShowMissingOnly((current) => !current)} />
      ...
      <GlassPanel><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[.16em] text-cyan-700">Funnel adımları</p><h3 className="mt-2 text-xl font-black text-[var(--admin-text-primary)]">{selectedCompany?.name || "Genel müşteri"} · {goal} · {channel}</h3></div><div className="flex flex-wrap gap-2"><button onClick={() => setShowMissingOnly((current) => !current)} className={`rounded-full border px-4 py-2 text-xs font-black ${showMissingOnly ? "border-amber-400 bg-amber-400 text-[var(--admin-text-primary)]" : "border-amber-200 bg-amber-50 text-amber-800"}`}>{showMissingOnly ? `Tümünü Göster (${steps.length})` : `Eksikleri Göster: ${missing.length}`}</button><button disabled title="Bu planlayıcı için AI öneri entegrasyonu henüz bağlı değil." className="cursor-not-allowed rounded-full bg-slate-200 px-4 py-2 text-xs font-black text-[var(--admin-text-muted)]">Yapay Zekâ Plan Öner (yakında)</button><button onClick={() => setActive?.("Müşteri Raporları")} className="rounded-full border border-[var(--admin-border)] bg-[var(--admin-surface)] px-4 py-2 text-xs font-black text-[var(--admin-text-secondary)]">Raporla / PDF</button></div></div><div className="mt-4 grid gap-3 md:grid-cols-3">{visibleSteps.map((item, index) => <FunnelStepCard key={item.step} step={item.step} index={index} status={item.status} action={item.action} />)}</div>{showMissingOnly && !missing.length && <p className="mt-3 rounded-[10px] border border-dashed border-[var(--admin-border)] p-4 text-center text-sm font-semibold text-[var(--admin-text-muted)]">Eksik adım yok.</p>}<pre className="mt-4 whitespace-pre-wrap rounded-[16px] bg-slate-950 p-4 text-xs leading-6 text-slate-100">{`${selectedCompany?.name || "Müşteri"} için ${goal} odaklı ${channel} funnel planı hazırlandı.\nEksikler: ${missing.map((item) => item.step).join(", ") || "Eksik yok"}.\nPDF/rapor çıktısı için Rapor Merkezi’nde bu plan metni kullanılabilir.`}</pre></GlassPanel>
    </div>
  );
}
...
export function GrowthMarketplaceCenter({ setActive }: GrowthProps) {
  ...
  return (
    <div className="grid gap-5">
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{packages.map((item, index) => <GlassPanel key={item} tone={index % 3 === 0 ? "purple" : index % 3 === 1 ? "cyan" : "emerald"}><PackageCheck className="text-cyan-700" size={24} /><h3 className="mt-3 text-lg font-black text-[var(--admin-text-primary)]">{item}</h3><p className="mt-2 text-sm leading-6 text-[var(--admin-text-secondary)]">Kimler için uygun? {item.includes("Klinik") ? "Sağlık ve randevu odaklı işletmeler" : item.includes("Rapor") ? "Düzenli performans raporu isteyen ajans müşterileri" : "Lead, teklif ve reklam performansı büyütmek isteyen müşteriler"}.</p><span className="mt-3 inline-flex rounded-full bg-[var(--admin-surface)] px-3 py-1 text-xs font-black text-[var(--admin-text-secondary)] ring-1 ring-slate-200">Kurulum durumu: {index < 3 ? "Aktif" : index < 8 ? "Hazır" : "Yakında"}</span><div className="mt-4 flex flex-wrap gap-2"><button onClick={() => setActive?.("Büyüme Motoru")} className="rounded-full bg-cyan-500 px-4 py-2 text-xs font-black text-white">Bu paketten plan oluştur</button></div></GlassPanel>)}</div>
    </div>
  );
}
