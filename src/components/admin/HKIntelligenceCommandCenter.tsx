...
export function HKIntelligenceCommandCenter({ content, setActive, notify, initialView = "command" }: any) {
  ...
  return <div className="grid gap-5">
    <section className="rounded-[24px] border border-[var(--admin-border)] bg-[var(--admin-surface)] p-5 shadow-[0_16px_38px_rgba(15,23,42,.07)] sm:p-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div><p className="text-xs font-black uppercase tracking-[.16em] text-cyan-700">HK Intelligence</p><h1 className="mt-2 text-2xl font-black text-[var(--admin-text-primary)] sm:text-3xl">Ajans Command Center</h1><p className="mt-2 max-w-3xl text-sm leading-6 text-[var(--admin-text-secondary)]">CRM, tahsilat, görev, teklif, kampanya ve entegrasyon verilerini günlük karar sırasına dönüştürür.</p></div>
    ...
    {view === "command" && <>
      ...
      <section className="grid gap-5 xl:grid-cols-[1.2fr_.8fr]">
        ...
        <div className="rounded-[22px] border border-[var(--admin-border)] bg-[var(--admin-surface)] p-5 shadow-sm"><div className="flex items-center justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[.14em] text-cyan-700">Öneri Motoru</p><h2 className="mt-1 text-xl font-black text-[var(--admin-text-primary)]">Bugünün önerileri</h2></div><RefreshCw size={18} className="text-cyan-600" /></div><div className="mt-4 grid gap-3">{recommendations.map((item, index) => <div key={item} className="flex gap-3 rounded-[14px] bg-[var(--admin-surface-soft)] p-3"><span className="grid size-7 shrink-0 place-items-center rounded-full bg-cyan-100 text-xs font-black text-cyan-700">{index + 1}</span><p className="text-sm leading-6 text-[var(--admin-text-secondary)]">{item}</p></div>)}</div></div>
      </section>
      <section className="grid gap-5 xl:grid-cols-3"><div className="rounded-[22px] border border-[var(--admin-border)] bg-[var(--admin-surface)] p-5 shadow-sm xl:col-span-2"><div className="flex items-center justify-between"><div><h2 className="text-lg font-black text-[var(--admin-text-primary)]">Operasyon Takvimi</h2><p className="mt-1 text-sm text-[var(--admin-text-muted)]">Yaklaşan görev, tahsilat, teklif, kampanya ve rapor tarihleri.</p></div><button onClick={() => setActive("Takvim")} className="rounded-full bg-blue-50 px-3 py-2 text-xs font-black text-blue-700">Takvimi Aç</button></div><div className="mt-4 grid gap-2">{calendarItems.map((item: any) => <button key={item.id} onClick={() => setActive(item.target)} className="flex items-center gap-3 rounded-[13px] border border-slate-100 bg-[var(--admin-surface-soft)] p-3 text-left"><CalendarDays size={17} className="text-blue-600" /><span className="min-w-0 flex-1"><strong className="block truncate text-sm text-[var(--admin-text-primary)]">{item.title}</strong><span className="text-xs text-[var(--admin-text-muted)]">{item.type} · {dateLabel(item.date)}</span></span></button>)}{!calendarItems.length && <p className="rounded-[14px] border border-dashed border-[var(--admin-border)] p-4 text-sm text-[var(--admin-text-muted)]">Yaklaşan operasyon tarihi bulunmuyor.</p>}</div></div><div className="rounded-[22px] border border-[var(--admin-border)] bg-[var(--admin-surface)] p-5 shadow-sm"><h2 className="text-lg font-black text-[var(--admin-text-primary)]">Müşteri Sağlığı</h2><p className="mt-1 text-sm text-[var(--admin-text-muted)]">En düşük skorlu müşteriler önce gösterilir.</p><div className="mt-4 grid gap-3">{healthRows.slice(0, 6).map(({ company, health }: any) => <button key={company.id} onClick={() => setActive("Müşteriler")} className="rounded-[13px] border border-slate-100 p-3 text-left"><span className="flex items-center justify-between gap-2"><strong className="truncate text-sm text-[var(--admin-text-primary)]">{company.name}</strong><span className={`rounded-full px-2 py-1 text-[10px] font-black ring-1 ${statusClass(health.status)}`}>{health.score}/100</span></span><span className="mt-2 block text-xs text-[var(--admin-text-muted)]">{health.reasons[0]}</span></button>)}</div></div></section>
    </>}

    {view === "lifecycle" && <section className="grid gap-5"><div className="overflow-x-auto rounded-[22px] border border-[var(--admin-border)] bg-[var(--admin-surface)] p-5 shadow-sm"><h2 className="text-xl font-black text-[var(--admin-text-primary)]">Müşteri yaşam döngüsü pipeline</h2><p className="mt-1 text-sm text-[var(--admin-text-muted)]">Lead aşamasından referansa kadar tüm müşteri ilişkisini mevcut operasyon kayıtlarıyla izleyin.</p><div className="mt-5 grid min-w-[1100px] grid-cols-10 gap-2">{lifecycleStages.map((stage) => <div key={stage} className="rounded-[14px] border border-[var(--admin-border)] bg-[var(--admin-surface-soft)] p-3 text-center"><strong className="block text-xs text-[var(--admin-text-secondary)]">{stage}</strong><span className="mt-2 block text-2xl font-black text-cyan-700">{lifecycleRows.filter((item: any) => item.stage === stage).length}</span></div>)}</div></div><div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{lifecycleRows.map(({ company, stage }: any) => { const health = healthRows.find((item: any) => item.company.id === company.id)?.health; return <button key={company.id} onClick={() => setActive("Müşteriler")} className="rounded-[18px] border border-[var(--admin-border)] bg-[var(--admin-surface)] p-4 text-left shadow-sm transition hover:border-cyan-200"><div className="flex items-start justify-between gap-3"><span><strong className="block text-[var(--admin-text-primary)]">{company.name}</strong><span className="mt-1 block text-xs text-[var(--admin-text-muted)]">{company.sector || "Sektör belirtilmedi"}</span></span><span className={`rounded-full px-2.5 py-1 text-[10px] font-black ring-1 ${statusClass(health?.status)}`}>{health?.score || 0}/100</span></div><div className="mt-4 flex items-center gap-2 text-xs font-black text-cyan-700"><span className="rounded-full bg-cyan-50 px-3 py-1.5">{stage}</span><ArrowRight size={14} /></div></button>; })}</div></section>}

    {view === "profit" && <section className="grid gap-5"><div className="grid gap-3 md:grid-cols-4">{[["Toplam Tahsil Edilen", money(profitRows.reduce((sum: number, item: any) => sum + item.collected, 0))], ["Bekleyen", money(profitRows.reduce((sum: number, item: any) => sum + item.pending, 0))], ["Operasyon Maliyeti", money(profitRows.reduce((sum: number, item: any) => sum + item.operationCost, 0))], ["Tahmini Net", money(profitRows.reduce((sum: number, item: any) => sum + item.profit, 0))]].map(([label, value]) => <div key={label} className="rounded-[18px] border border-[var(--admin-border)] bg-[var(--admin-surface)] p-4 shadow-sm"><p className="text-xs font-bold text-[var(--admin-text-muted)]">{label}</p><p className="mt-2 text-2xl font-black text-[var(--admin-text-primary)]">{value}</p></div>)}</div><div className="overflow-x-auto rounded-[22px] border border-[var(--admin-border)] bg-[var(--admin-surface)] p-5 shadow-sm"><h2 className="text-xl font-black text-[var(--admin-text-primary)]">Müşteri bazlı kârlılık</h2><table className="mt-4 w-full min-w-[900px] text-left text-sm"><thead><tr className="border-b border-[var(--admin-border)] text-xs text-[var(--admin-text-muted)]">{["Müşteri", "Hizmet Bedeli", "Tahsil Edilen", "Bekleyen", "Reklam Harcaması", "Operasyon Maliyeti", "Tahmini Kârlılık"].map((item) => <th key={item} className="px-3 py-3">{item}</th>)}</tr></thead><tbody>{profitRows.map((item: any) => <tr key={item.company.id} className="border-b border-slate-100 hover:bg-[var(--admin-surface-soft)]"><td className="px-3 py-3 font-black text-[var(--admin-text-primary)]">{item.company.name}</td><td className="px-3 py-3">{money(item.serviceFee)}</td><td className="px-3 py-3 text-emerald-700">{money(item.collected)}</td><td className="px-3 py-3 text-amber-700">{money(item.pending)}</td><td className="px-3 py-3">{money(item.adSpend)}</td><td className="px-3 py-3">{money(item.operationCost)}</td><td className={`px-3 py-3 font-black ${item.profit >= 0 ? "text-emerald-700" : "text-red-700"}`}>{money(item.profit)}</td></tr>)}</tbody></table></div></section>}

    {view === "risk" && <section className="grid gap-5 xl:grid-cols-[1fr_340px]"><div className="rounded-[22px] border border-[var(--admin-border)] bg-[var(--admin-surface)] p-5 shadow-sm"><div className="flex items-center justify-between"><div><p className="text-xs font-black uppercase tracking-[.14em] text-red-600">Risk Merkezi</p><h2 className="mt-1 text-xl font-black text-[var(--admin-text-primary)]">Önceliklendirilmiş riskler</h2></div><ShieldAlert className="text-red-600" /></div><div className="mt-4 grid gap-3">{risks.map((risk) => <button key={risk.id} onClick={() => setActive(risk.target)} className="flex items-start gap-3 rounded-[15px] border border-[var(--admin-border)] p-4 text-left transition hover:border-red-200 hover:bg-red-50"><span className={`grid size-11 shrink-0 place-items-center rounded-[13px] font-black ${risk.priority >= 90 ? "bg-red-100 text-red-700" : risk.priority >= 80 ? "bg-orange-100 text-orange-700" : "bg-amber-100 text-amber-700"}`}>{risk.priority}</span><span className="min-w-0 flex-1"><strong className="block text-sm text-[var(--admin-text-primary)]">{risk.title}</strong><span className="mt-1 block text-xs leading-5 text-[var(--admin-text-muted)]">{risk.module} · {risk.detail}</span></span><ArrowRight size={16} className="mt-2 shrink-0 text-slate-400" /></button>)}{!risks.length && <div className="rounded-[16px] border border-dashed border-emerald-200 bg-emerald-50 p-5 text-sm text-emerald-700">Kritik risk sinyali bulunmuyor.</div>}</div></div><div className="grid content-start gap-4">{[["Geciken Tahsilat", overduePayments.length, "Tahsilat"], ["Pixel Sorunu", pixelRisks.length, "Entegrasyonlar"], ["Entegrasyon Hatası", integrationRisks.length, "Entegrasyonlar"], ["Kritik Görev", criticalTasks.length, "Görevler"], ["İletişimsiz Müşteri", healthRows.filter((item: any) => item.health.reasons.some((reason: string) => reason.includes("temas"))).length, "Müşteriler"]].map(([label, count, target]: any) => <button key={label} onClick={() => setActive(target)} className="flex items-center justify-between rounded-[16px] border border-[var(--admin-border)] bg-[var(--admin-surface)] p-4 text-left shadow-sm"><span className="flex items-center gap-3"><HeartPulse size={18} className="text-red-600" /><strong className="text-sm text-slate-800">{label}</strong></span><span className="rounded-full bg-red-50 px-3 py-1 text-sm font-black text-red-700">{count}</span></button>)}</div></section>}
  </div>;
}
