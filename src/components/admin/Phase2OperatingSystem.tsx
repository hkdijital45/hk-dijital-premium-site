...
function Card({ title, description, icon: Icon, children }: any) {
  return <section className="rounded-[8px] border border-[var(--admin-border)] bg-[var(--admin-surface)] p-5 shadow-[0_24px_80px_rgba(15,23,42,.24)]"><div className="mb-4 flex items-start gap-3"><div className="grid size-10 place-items-center rounded-[8px] bg-cyan-300/15 text-cyan-700"><Icon size={19} /></div><div><h2 className="font-black text-[var(--admin-text-primary)]">{title}</h2>{description && <p className="mt-1 text-sm leading-6 text-slate-400">{description}</p>}</div></div>{children}</section>;
}

function Field({ label, value, onChange, type = "text", placeholder = "" }: any) {
  return <label className="grid gap-2 text-xs font-bold text-[var(--admin-text-secondary)]">{label}<input type={type} value={value || ""} placeholder={placeholder} onChange={(event) => onChange(event.target.value)} className="min-h-11 rounded-[8px] border border-[var(--admin-border)] bg-[var(--admin-surface)] px-3 text-sm text-[var(--admin-text-primary)] outline-none focus:border-cyan-200/60" /></label>;
}

function Select({ label, value, onChange, options }: any) {
  return <label className="grid gap-2 text-xs font-bold text-[var(--admin-text-secondary)]">{label}<select value={value || ""} onChange={(event) => onChange(event.target.value)} className="min-h-11 rounded-[8px] border border-[var(--admin-border)] bg-[var(--admin-surface)] px-3 text-sm text-[var(--admin-text-primary)] outline-none focus:border-cyan-200/60">{options.map((item: any) => <option key={item.value || item} value={item.value || item}>{item.label || item}</option>)}</select></label>;
}

function PageShell({ eyebrow, title, description, children }: any) {
  return <main className="min-h-screen bg-[var(--admin-surface-soft)] px-4 py-8 text-[var(--admin-text-primary)]"><div className="mx-auto max-w-7xl"><p className="text-xs font-black uppercase tracking-[.18em] text-cyan-700">{eyebrow}</p><h1 className="mt-3 text-3xl font-black">{title}</h1><p className="mt-3 max-w-4xl text-sm leading-7 text-[var(--admin-text-secondary)]">{description}</p><div className="mt-7">{children}</div></div></main>;
}
...
export function IntegrationCenter({ provider, content, integrations = [] }: any) {
  ...
  const canSyncForm = Boolean(form.accessToken.trim() || items.some((item: any) => item.hasAccessToken));
  async function save() {
    ...
    <div className="grid gap-5 xl:grid-cols-[.95fr_1.05fr]">
      <Card title="Bağlantı Bilgileri" description="Canlı OAuth/app izinleri tamamlandığında bu kayıtlar otomatik raporlama sync akışına bağlanır." icon={ShieldCheck}>
        <div className="grid gap-3 md:grid-cols-2">
          ...
          <label className="flex items-center gap-2 text-sm font-bold text-[var(--admin-text-secondary)]"><input type="checkbox" checked={form.autoSync} onChange={(event) => setForm({ ...form, autoSync: event.target.checked })} /> Otomatik senkronizasyon açık</label>
        </div>
        <div className="mt-4 flex flex-wrap gap-2"><button type="button" onClick={save} className="rounded-full bg-cyan-300 px-5 py-3 text-xs font-black text-[var(--admin-text-primary)]">Bağlantıyı Kaydet</button><button type="button" disabled={!canSyncForm} title={!canSyncForm ? "Senkronizasyon için API anahtarı veya kayıtlı token gerekli." : undefined} onClick={() => sync()} className="inline-flex items-center gap-2 rounded-full border border-cyan-200/30 px-5 py-3 text-xs font-black text-cyan-700 disabled:cursor-not-allowed disabled:opacity-50"><RefreshCw size={14} /> Manuel Sync</button></div>
        {!canSyncForm && <p className="mt-2 text-xs font-bold text-amber-700">API anahtarı veya kayıtlı token olmadan canlı senkronizasyon başlatılamaz.</p>}
      ...
      <Card title="Bağlantı Durumu" description="Tokenlar maskelenir; sadece bağlantı durumu ve var/yok bilgisi gösterilir." icon={Link2}>
        <div className="grid gap-3">{items.length ? items.map((item: any) => <div key={item.id} className="rounded-[8px] border border-[var(--admin-border)] bg-[var(--admin-surface-soft)] p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-black">{item.provider?.toUpperCase()} · {item.ad_account_id || item.business_account_id || "Hesap bekliyor"}</p><p className="mt-1 text-xs text-slate-400">Son senkronizasyon: {item.last_sync_at ? new Date(item.last_sync_at).toLocaleString("tr-TR") : "Henüz yok"} · Yetkilendirme: {item.hasAccessToken ? "Hazır" : "Eksik"}</p></div><span className="rounded-full border border-cyan-200/20 bg-cyan-200/10 px-3 py-1 text-xs font-black text-cyan-700">{item.status || "Bekliyor"}</span></div><button type="button" disabled={!item.hasAccessToken} title={!item.hasAccessToken ? "Bu bağlantı için kayıtlı API yetkisi gerekli." : undefined} onClick={() => sync(item.id)} className="mt-3 rounded-full border border-[var(--admin-border)] px-4 py-2 text-xs font-bold disabled:cursor-not-allowed disabled:opacity-50">Bağlantıyı Senkronize Et</button>{!item.hasAccessToken && <p className="mt-2 text-xs text-amber-700">API yetkilendirmesi gerekli.</p>}</div>) : <p className="rounded-[8px] border border-dashed border-[var(--admin-border)] p-4 text-sm text-[var(--admin-text-muted)]">Henüz bağlantı kaydı yok. Önce müşteri hesabı için Meta veya Google bağlantısı ekleyin.</p>}</div>
      </Card>
    </div>
  </PageShell>;
}
...
export function OnboardingCenter({ content }: any) {
  ...
  return <PageShell eyebrow="Müşteri Başarısı" title="Müşteri Başlangıç Merkezi" description="Doğrulanan lead kaydını müşteriye dönüştürün; firma ve giriş hesabı tek işlemde oluşturulsun.">
    ...
    {createdCompany?.id && <Card title="Sıradaki Kurulum Adımları" description={`${createdCompany.name} oluşturuldu. Tamamlanan adımlar müşteri profilindeki kurulum rehberinde otomatik işaretlenir.`} icon={CheckCircle2}>
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        ...
        ].map(([order, label, tab]) => <a key={label} href={`/hk-admin/musteriler?companyId=${createdCompany.id}&tab=${tab}`} className="rounded-[14px] border border-cyan-200 bg-cyan-50 p-4 transition hover:border-cyan-400 hover:bg-[var(--admin-surface)]"><span className="grid size-8 place-items-center rounded-full bg-cyan-500 text-xs font-black text-white">{order}</span><strong className="mt-3 block text-sm text-[var(--admin-text-primary)]">{label}</strong><span className="mt-1 block text-xs leading-5 text-[var(--admin-text-secondary)]">Müşteri profilinde bu adıma devam et.</span></a>)}
      </div>
    </Card>}
    <div className="grid gap-5 xl:grid-cols-2">
      <Card title="Müşteriye Dönüştürülebilecek Leadler" description="Bilgilerini doğruladığınız lead için firma ve müşteri giriş hesabını oluşturun." icon={Users}>
        <div className="grid gap-3">{leads.length ? leads.slice(0, 20).map((lead: any) => <div key={lead.id} className="rounded-[8px] border border-[var(--admin-border)] bg-[var(--admin-surface-soft)] p-4"><div className="flex flex-wrap justify-between gap-3"><div><p className="font-black">{lead.company || lead.name || "İsimsiz lead"}</p><p className="mt-1 text-xs text-slate-400">{lead.email || "E-posta yok"} · {lead.phone || "Telefon yok"} · {lead.status || "Yeni"}</p></div><button disabled={Boolean(loadingLeadId)} onClick={() => convert(lead.id)} className="rounded-full bg-cyan-300 px-4 py-2 text-xs font-black text-[var(--admin-text-primary)] disabled:cursor-not-allowed disabled:opacity-60">{loadingLeadId === lead.id ? "Oluşturuluyor..." : "Doğrula ve Müşteri Oluştur"}</button></div></div>) : <p className="rounded-[8px] border border-dashed border-[var(--admin-border)] p-4 text-sm text-[var(--admin-text-muted)]">Dönüştürülebilecek lead bulunmuyor. Yeni başvurular Lead Merkezi ekranında göründüğünde burada işleme alınabilir.</p>}</div>
      </Card>
      <Card title="Aktif Müşteri Hesapları" description="Mevcut müşteri login kayıtları." icon={CheckCircle2}>
        <div className="grid gap-3">{customers.length ? customers.map((user: any) => <div key={user.id} className="flex flex-wrap items-center justify-between gap-3 rounded-[8px] border border-[var(--admin-border)] bg-[var(--admin-surface-soft)] p-4"><div><p className="font-black">{user.full_name || user.email}</p><p className="mt-1 text-xs text-slate-400">{user.email} · {user.is_active ? "Aktif" : "Pasif"}</p></div>{user.company_id && <a href={`/hk-admin/musteriler?companyId=${user.company_id}`} className="rounded-[8px] border border-cyan-200 bg-[var(--admin-surface)] px-3 py-2 text-xs font-black text-cyan-700">Müşteri Profilini Aç</a>}</div>) : <p className="rounded-[8px] border border-dashed border-[var(--admin-border)] p-4 text-sm text-[var(--admin-text-muted)]">Henüz müşteri giriş hesabı oluşturulmadı.</p>}</div>
      </Card>
    </div>
  </PageShell>;
}
...
export function LeadWorkspace({ content, canManageTestRecords = false }: any) {
  ...
  return <PageShell eyebrow="Lead Yönetimi" title="Lead Çalışma Alanı" description="Keşif, yapay zekâ analizi, dijital skor, teklif geçmişi, müşteri ilişkileri durumu ve müşteri dönüşümünü tek satış sürecinde izleyin.">
    <div className="mb-5 flex flex-wrap items-center justify-between gap-3 rounded-[8px] border border-[var(--admin-border)] bg-[var(--admin-surface)] p-4"><div><p className="font-black text-[var(--admin-text-primary)]">Kayıt görünümü</p><p className="mt-1 text-xs text-[var(--admin-text-muted)]">Test kayıtları canlı satış ve dönüşüm istatistiklerine dahil edilmez.</p></div><div className="flex flex-wrap gap-2">{([['live','Gerçek Kayıtlar'],['test','Test Kayıtları'],['all','Tümü']] as const).map(([value,label]) => <button type="button" key={value} onClick={() => setRecordVisibility(value)} className={`min-h-10 rounded-[8px] px-4 text-xs font-black ${recordVisibility === value ? 'bg-cyan-500 text-white' : 'border border-[var(--admin-border)] bg-[var(--admin-surface)] text-[var(--admin-text-secondary)]'}`}>{label}</button>)}</div></div>
    ...
    <div className="mb-5 grid gap-3 md:grid-cols-4">{[["Toplam lead", summary.totalLeads], ["Aktif müşteri", summary.activeCustomers], ["Teklif değeri", formatCurrency(summary.proposalValue)], ["Dönüşüm oranı", `%${formatNumber(summary.conversionRate, 1)}`]].map(([label, value]) => <div key={label} className="rounded-[8px] border border-[var(--admin-border)] bg-[var(--admin-surface)] p-4"><p className="text-xs text-[var(--admin-text-muted)]">{label}</p><p className="mt-2 text-2xl font-black">{value}</p></div>)}</div>
    ...
    <div className="grid gap-5 xl:grid-cols-[1fr_360px]">
      <div className="grid gap-4 xl:grid-cols-3">{stages.map((stage) => <div key={stage} className="rounded-[8px] border border-[var(--admin-border)] bg-[var(--admin-surface)] p-3"><div className="flex items-center justify-between gap-2"><h3 className="font-black text-cyan-700">{stage}</h3><span className="rounded-full bg-slate-100 px-2 py-1 text-[10px] font-black text-[var(--admin-text-secondary)]">{visibleLeads.filter((lead: any) => leadStage(lead) === stage).length}</span></div><div className="mt-3 grid gap-3">{visibleLeads.filter((lead: any) => leadStage(lead) === stage).slice(0, 8).map((lead: any) => <div key={lead.id} className="rounded-[8px] bg-[var(--admin-surface-soft)] p-3"><div className="flex items-start justify-between gap-2"><p className="font-black">{lead.company || lead.name || "Lead"}</p>{isTestRecord(lead) && <span className="rounded-full bg-violet-100 px-2 py-1 text-[10px] font-black text-violet-700">TEST</span>}</div><p className="mt-1 text-xs text-[var(--admin-text-muted)]">{lead.business_type || lead.sector || "-"} · {lead.city || "-"}</p><div className="mt-2 flex flex-wrap gap-2 text-[10px] font-black"><span className="rounded-full bg-cyan-300/15 px-2 py-1 text-cyan-700">Dijital {lead.digital_maturity_score || calculateDigitalMaturityScore(lead)}</span><span className="rounded-full bg-amber-300/15 px-2 py-1 text-amber-700">Isı {lead.lead_heat_score || calculateLeadHeatScore(lead)}</span></div><select disabled={pendingLeadId === lead.id} value={leadStage(lead)} onChange={(event) => moveLead(lead.id, event.target.value)} className="mt-3 min-h-10 w-full rounded-[8px] border border-[var(--admin-border)] bg-[var(--admin-surface)] px-2 text-xs font-bold text-[var(--admin-text-primary)] outline-none disabled:opacity-60"><option value="Yeni Lead">Yeni Lead</option><option value="İletişim Kuruldu">İletişim Kuruldu</option><option value="Teklif Gönderildi">Teklif Gönderildi</option><option value="Takipte">Takipte</option><option value="Kazandı">Kazandı</option><option value="Kaybedildi">Kaybedildi</option></select>{canManageTestRecords && <button type="button" disabled={pendingLeadId === lead.id} onClick={() => toggleTestRecord(lead)} className="mt-2 min-h-10 w-full rounded-[8px] border border-violet-200 px-3 text-xs font-black text-violet-700 disabled:opacity-60">{isTestRecord(lead) ? "Gerçek kayda dönüştür" : "Test kaydı yap"}</button>}</div>)}</div></div>)}</div>
      <Card title="HK Intelligence Asistanı" description="Operasyon sorularını mevcut lead, müşteri, kampanya ve rapor bağlamıyla yanıtlar." icon={Bot}><textarea aria-label="Operasyon sorusu" value={question} onChange={(event) => setQuestion(event.target.value)} className="min-h-24 w-full rounded-[8px] border border-[var(--admin-border)] bg-[var(--admin-surface-soft)] p-3 text-sm text-[var(--admin-text-primary)]" /><button type="button" disabled={asking || !question.trim()} onClick={ask} className="mt-3 inline-flex min-h-11 items-center gap-2 rounded-full bg-cyan-300 px-4 text-sm font-black text-[var(--admin-text-primary)] disabled:opacity-60"><Sparkles size={14} /> {asking ? "Yanıt hazırlanıyor..." : "Sor"}</button>{answer && <p className="mt-4 whitespace-pre-line rounded-[8px] bg-[var(--admin-surface-soft)] p-3 text-sm leading-6 text-[var(--admin-text-secondary)]">{answer}</p>}</Card>
    </div>
  </PageShell>;
}
  ...
  return <PageShell eyebrow="Satış Sistemi" title="Akıllı Teklif Oluşturucu" description="İşletme bilgilerini girin; paket, satış hunisi, performans tahmini ve Türkçe satış açıklaması oluşturun.">
    <div className="grid gap-5 xl:grid-cols-[.9fr_1.1fr]">
      <Card title="Teklif Girdileri" description="Admin göndermeden önce tüm metni düzenleyebilir." icon={FileText}>
        ...
        <button onClick={generate} className="mt-4 inline-flex items-center gap-2 rounded-full bg-cyan-300 px-5 py-3 text-xs font-black text-[var(--admin-text-primary)]"><Send size={14} /> Teklif Oluştur</button>
      </Card>
      <Card title="Teklif Önizleme" description="Tahminler satış garantisi değil, planlama öngörüsüdür." icon={BarChart3}>
        <div className="grid gap-3 md:grid-cols-5">{[["Erişim", preview.reach], ["Tıklama", preview.clicks], ["Lead", preview.leads], ["Mesaj", preview.messages], ["CPL", formatCurrency(preview.costPerLead)]].map(([label, value]) => <div key={label} className="rounded-[8px] bg-[var(--admin-surface-soft)] p-3"><p className="text-xs text-slate-400">{label}</p><p className="mt-1 text-lg font-black">{typeof value === "number" ? formatNumber(value, 0) : value}</p></div>)}</div>
        {proposal && <div className="mt-5 rounded-[8px] border border-cyan-200/20 bg-cyan-200/10 p-4"><p className="font-black text-cyan-700">Paket: {proposal.packageName || "-"}</p><p className="mt-2 text-sm text-cyan-700">Funnel: {(proposal.funnel || []).join(" -> ")}</p><textarea defaultValue={proposal.explanation || proposal.error || ""} className="mt-4 min-h-56 w-full rounded-[8px] border border-[var(--admin-border)] bg-[var(--admin-surface-soft)] p-3 text-sm leading-6 text-[var(--admin-text-primary)]" /></div>}
      </Card>
    </div>
  </PageShell>;
}
