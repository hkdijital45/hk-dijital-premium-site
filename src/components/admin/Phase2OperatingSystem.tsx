  ...
  return <PageShell eyebrow="HK Operating System" title={isMeta ? "Meta Integration Center" : "Google Ads Integration Center"} description="Reklam hesap bağlantılarını server-side saklayın, manuel veya zamanlanmış sync uçlarını kullanın. Token değerleri tarayıcıya geri döndürülmez.">
    <div className="grid gap-5 xl:grid-cols-[.95fr_1.05fr]">
      <Card title="Bağlantı Bilgileri" description="Canlı OAuth/app izinleri tamamlandığında bu kayıtlar otomatik raporlama sync akışına bağlanır." icon={ShieldCheck}>
        ...
        <div className="mt-4 flex flex-wrap gap-2"><button onClick={save} className="rounded-full bg-cyan-300 px-5 py-3 text-xs font-black text-slate-950">Bağlantıyı Kaydet</button><button onClick={() => sync()} className="inline-flex items-center gap-2 rounded-full border border-cyan-200/30 px-5 py-3 text-xs font-black text-cyan-700"><RefreshCw size={14} /> Manuel Sync</button></div>
      ...
      <Card title="Bağlantı Durumu" description="Tokenlar maskelenir; sadece bağlantı durumu ve var/yok bilgisi gösterilir." icon={Link2}>
        <div className="grid gap-3">{items.length ? items.map((item: any) => <div key={item.id} className="rounded-[8px] border border-slate-200 bg-slate-50 p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-black">{item.provider?.toUpperCase()} · {item.ad_account_id || item.business_account_id || "Hesap bekliyor"}</p><p className="mt-1 text-xs text-slate-400">Son sync: {item.last_sync_at ? new Date(item.last_sync_at).toLocaleString("tr-TR") : "Henüz yok"} · Token: {item.hasAccessToken ? "Var" : "Yok"}</p></div><span className="rounded-full border border-cyan-200/20 bg-cyan-200/10 px-3 py-1 text-xs font-black text-cyan-700">{item.status || "Bekliyor"}</span></div><button onClick={() => sync(item.id)} className="mt-3 rounded-full border border-slate-200 px-4 py-2 text-xs font-bold">Bu Bağlantıyı Sync Et</button></div>) : <p className="text-sm text-slate-400">Henüz bağlantı kaydı yok.</p>}</div>
      </Card>
    </div>
  </PageShell>;
}
