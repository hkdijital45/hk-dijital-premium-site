...
function Card({ title, description, icon: Icon, children }: any) {
  return <section className="rounded-[8px] border border-slate-200 bg-white p-5 shadow-[0_24px_80px_rgba(15,23,42,.24)]"><div className="mb-4 flex items-start gap-3"><div className="grid size-10 place-items-center rounded-[8px] bg-cyan-300/15 text-cyan-700"><Icon size={19} /></div><div><h2 className="font-black text-slate-900">{title}</h2>{description && <p className="mt-1 text-sm leading-6 text-slate-400">{description}</p>}</div></div>{children}</section>;
}

function Field({ label, value, onChange, type = "text", placeholder = "" }: any) {
  return <label className="grid gap-2 text-xs font-bold text-slate-600">{label}<input type={type} value={value || ""} placeholder={placeholder} onChange={(event) => onChange(event.target.value)} className="min-h-11 rounded-[8px] border border-slate-200 bg-white px-3 text-sm text-slate-900 outline-none focus:border-cyan-200/60" /></label>;
}

function Select({ label, value, onChange, options }: any) {
  return <label className="grid gap-2 text-xs font-bold text-slate-600">{label}<select value={value || ""} onChange={(event) => onChange(event.target.value)} className="min-h-11 rounded-[8px] border border-slate-200 bg-white px-3 text-sm text-slate-900 outline-none focus:border-cyan-200/60">{options.map((item: any) => <option key={item.value || item} value={item.value || item}>{item.label || item}</option>)}</select></label>;
}

function PageShell({ eyebrow, title, description, children }: any) {
  return <main className="min-h-screen bg-slate-50 px-4 py-8 text-slate-900"><div className="mx-auto max-w-7xl"><p className="text-xs font-black uppercase tracking-[.18em] text-cyan-700">{eyebrow}</p><h1 className="mt-3 text-3xl font-black">{title}</h1><p className="mt-3 max-w-4xl text-sm leading-7 text-slate-600">{description}</p><div className="mt-7">{children}</div></div></main>;
}
  ...
  return <PageShell eyebrow="HK Operating System" title={isMeta ? "Meta Integration Center" : "Google Ads Integration Center"} description="Reklam hesap bağlantılarını server-side saklayın, manuel veya zamanlanmış sync uçlarını kullanın. Token değerleri tarayıcıya geri döndürülmez.">
    <div className="grid gap-5 xl:grid-cols-[.95fr_1.05fr]">
      ...
      <Card title="Bağlantı Durumu" description="Tokenlar maskelenir; sadece bağlantı durumu ve var/yok bilgisi gösterilir." icon={Link2}>
        <div className="grid gap-3">{items.length ? items.map((item: any) => <div key={item.id} className="rounded-[8px] border border-slate-200 bg-slate-50 p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-black">{item.provider?.toUpperCase()} · {item.ad_account_id || item.business_account_id || "Hesap bekliyor"}</p><p className="mt-1 text-xs text-slate-400">Son senkronizasyon: {item.last_sync_at ? new Date(item.last_sync_at).toLocaleString("tr-TR") : "Henüz yok"} · Yetkilendirme: {item.hasAccessToken ? "Hazır" : "Eksik"}</p></div><span className="rounded-full border border-cyan-200/20 bg-cyan-200/10 px-3 py-1 text-xs font-black text-cyan-700">{item.status || "Bekliyor"}</span></div><button type="button" disabled={!item.hasAccessToken} title={!item.hasAccessToken ? "Bu bağlantı için kayıtlı API yetkisi gerekli." : undefined} onClick={() => sync(item.id)} className="mt-3 rounded-full border border-slate-200 px-4 py-2 text-xs font-bold disabled:cursor-not-allowed disabled:opacity-50">Bağlantıyı Senkronize Et</button>{!item.hasAccessToken && <p className="mt-2 text-xs text-amber-700">API yetkilendirmesi gerekli.</p>}</div>) : <p className="rounded-[8px] border border-dashed border-slate-200 p-4 text-sm text-slate-500">Henüz bağlantı kaydı yok. Önce müşteri hesabı için Meta veya Google bağlantısı ekleyin.</p>}</div>
      </Card>
    </div>
  </PageShell>;
}
  ...
  return <PageShell eyebrow="Müşteri Başarısı" title="Müşteri Başlangıç Merkezi" description="Doğrulanan lead kaydını müşteriye dönüştürün; firma ve giriş hesabı tek işlemde oluşturulsun.">
    ...
    <div className="grid gap-5 xl:grid-cols-2">
      <Card title="Müşteriye Dönüştürülebilecek Leadler" description="Bilgilerini doğruladığınız lead için firma ve müşteri giriş hesabını oluşturun." icon={Users}>
        <div className="grid gap-3">{leads.length ? leads.slice(0, 20).map((lead: any) => <div key={lead.id} className="rounded-[8px] border border-slate-200 bg-slate-50 p-4"><div className="flex flex-wrap justify-between gap-3"><div><p className="font-black">{lead.company || lead.name || "İsimsiz lead"}</p><p className="mt-1 text-xs text-slate-400">{lead.email || "E-posta yok"} · {lead.phone || "Telefon yok"} · {lead.status || "Yeni"}</p></div><button disabled={Boolean(loadingLeadId)} onClick={() => convert(lead.id)} className="rounded-full bg-cyan-300 px-4 py-2 text-xs font-black text-slate-950 disabled:cursor-not-allowed disabled:opacity-60">{loadingLeadId === lead.id ? "Oluşturuluyor..." : "Doğrula ve Müşteri Oluştur"}</button></div></div>) : <p className="rounded-[8px] border border-dashed border-slate-200 p-4 text-sm text-slate-500">Dönüştürülebilecek lead bulunmuyor. Yeni başvurular Lead Merkezi ekranında göründüğünde burada işleme alınabilir.</p>}</div>
      </Card>
      <Card title="Aktif Müşteri Hesapları" description="Mevcut müşteri login kayıtları." icon={CheckCircle2}>
        <div className="grid gap-3">{customers.length ? customers.map((user: any) => <div key={user.id} className="flex flex-wrap items-center justify-between gap-3 rounded-[8px] border border-slate-200 bg-slate-50 p-4"><div><p className="font-black">{user.full_name || user.email}</p><p className="mt-1 text-xs text-slate-400">{user.email} · {user.is_active ? "Aktif" : "Pasif"}</p></div>{user.company_id && <a href={`/hk-admin/musteriler?companyId=${user.company_id}`} className="rounded-[8px] border border-cyan-200 bg-white px-3 py-2 text-xs font-black text-cyan-700">Müşteri Profilini Aç</a>}</div>) : <p className="rounded-[8px] border border-dashed border-slate-200 p-4 text-sm text-slate-500">Henüz müşteri giriş hesabı oluşturulmadı.</p>}</div>
      </Card>
    </div>
  </PageShell>;
}
  ...
  return <PageShell eyebrow="Sales OS" title="Smart Proposal Generator" description="İşletme bilgilerini girin; paket, funnel, KPI tahmini ve Türkçe satış açıklaması oluşturun.">
    <div className="grid gap-5 xl:grid-cols-[.9fr_1.1fr]">
      ...
      <Card title="Teklif Önizleme" description="Tahminler satış garantisi değil, planlama öngörüsüdür." icon={BarChart3}>
        <div className="grid gap-3 md:grid-cols-5">{[["Erişim", preview.reach], ["Tıklama", preview.clicks], ["Lead", preview.leads], ["Mesaj", preview.messages], ["CPL", formatCurrency(preview.costPerLead)]].map(([label, value]) => <div key={label} className="rounded-[8px] bg-slate-50 p-3"><p className="text-xs text-slate-400">{label}</p><p className="mt-1 text-lg font-black">{typeof value === "number" ? formatNumber(value, 0) : value}</p></div>)}</div>
        {proposal && <div className="mt-5 rounded-[8px] border border-cyan-200/20 bg-cyan-200/10 p-4"><p className="font-black text-cyan-700">Paket: {proposal.packageName || "-"}</p><p className="mt-2 text-sm text-cyan-700">Funnel: {(proposal.funnel || []).join(" -> ")}</p><textarea defaultValue={proposal.explanation || proposal.error || ""} className="mt-4 min-h-56 w-full rounded-[8px] border border-slate-200 bg-slate-50 p-3 text-sm leading-6 text-slate-900" /></div>}
      </Card>
    </div>
  </PageShell>;
}
