...
export function SystemGuideCenter({ currentSession, notify }: any) {
  ...
  return <div className="grid min-w-0 gap-5 overflow-x-hidden">
    <section className="min-w-0 rounded-[22px] border border-cyan-200 bg-gradient-to-br from-cyan-50 via-white to-blue-50 p-4 shadow-[0_12px_35px_rgba(15,23,42,.07)] sm:p-6">
      <div className="flex flex-wrap items-start justify-between gap-4"><div className="min-w-0"><p className="text-xs font-black uppercase tracking-[.16em] text-cyan-700">Yaşayan bilgi merkezi</p><h2 className="mt-2 text-3xl font-black text-slate-950">📚 HK Dijital Sistem Rehberi</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">Tüm modüllerin kullanım kılavuzu, eğitim içerikleri ve sorun giderme rehberleri. İçerikler aranabilir, favorilenebilir ve admin tarafından geliştirilebilir.</p></div>{isAdmin && <button onClick={() => setEditor({ ...emptyGuide, category: categories[0]?.name || "" })} className="inline-flex items-center gap-2 rounded-[14px] bg-cyan-500 px-4 py-3 text-sm font-black text-white"><Plus size={16} /> Yeni Rehber</button>}</div>
