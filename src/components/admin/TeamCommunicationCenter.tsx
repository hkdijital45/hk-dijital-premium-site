  ...
  return <div className="communication-center min-w-0 space-y-4">
    <section className="communication-card rounded-[18px] border border-slate-200 bg-white p-4 shadow-sm">
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-[minmax(0,1fr)_170px_150px_130px_auto]">
        ...
        <button type="button" onClick={() => setUnreadOnly((current) => !current)} className={`min-h-11 rounded-[10px] px-3 text-xs font-black ${unreadOnly ? "bg-cyan-700 text-white shadow-sm" : "border border-slate-200 bg-white text-slate-700 hover:bg-cyan-50"}`}>Okunmamış</button>
    ...
    <section className="communication-layout grid min-h-[680px] overflow-hidden rounded-[18px] border border-slate-200 bg-white shadow-sm xl:grid-cols-[330px_minmax(0,1fr)_360px] 2xl:grid-cols-[350px_minmax(0,1fr)_390px]">
      <aside className="min-h-0 border-b border-slate-200 bg-slate-50/80 p-3 xl:border-b-0 xl:border-r">
        <div className="max-h-[720px] space-y-2 overflow-y-auto pr-1">{items.map((item) => <button type="button" key={item.id} onClick={() => setSelectedId(item.id)} className={`w-full rounded-[14px] border p-3 text-left transition ${selectedId === item.id ? "border-cyan-400 bg-cyan-50 shadow-sm ring-2 ring-cyan-100" : item.priority === "urgent" ? "border-rose-200 bg-rose-50 hover:border-rose-300" : "border-slate-200 bg-white hover:border-cyan-200 hover:bg-slate-50"}`}>
          ...
          <div className="mt-3 flex flex-wrap gap-1.5"><span className="rounded-full bg-slate-100 px-2 py-1 text-[10px] font-black text-slate-700">{label(statusOptions, item.status)}</span><span className="rounded-full bg-amber-100 px-2 py-1 text-[10px] font-black text-amber-800">{label(priorityOptions, item.priority)}</span></div>
      ...
      <main className="flex min-h-0 min-w-0 flex-col">
        ...
        {busy !== "detail" && detail && <>
          ...
          <div className="min-h-0 flex-1 space-y-4 overflow-y-auto p-4">{detail.messages.map((item) => {
            ...
            return <article key={item.id} className={`communication-message rounded-[16px] border p-4 shadow-sm ${own ? "border-cyan-200 bg-cyan-50" : "border-slate-200 bg-white"}`}>
          ...
          <footer className="communication-composer border-t border-slate-200 bg-white p-4">
            ...
            <div className="mt-3 flex flex-wrap items-center gap-3"><label className="inline-flex min-h-11 cursor-pointer items-center gap-2 rounded-[10px] border border-slate-300 bg-white px-3 text-xs font-black text-slate-700"><FileUp size={15} />{file ? file.name : "Dosya ekle"}<input type="file" className="sr-only" accept=".jpg,.jpeg,.png,.webp,.pdf,.doc,.docx,.xls,.xlsx" onChange={(event) => setFile(event.target.files?.[0] || null)} /></label><button type="button" onClick={sendMessage} disabled={!reply.trim() || busy === "reply"} className="inline-flex min-h-11 items-center gap-2 rounded-[10px] bg-cyan-700 px-5 text-sm font-black text-white shadow-sm hover:bg-cyan-800 disabled:bg-slate-200 disabled:text-slate-600">{busy === "reply" ? <Loader2 size={17} className="animate-spin" /> : <Send size={17} />} Gönder</button></div>
          </footer>
        </>}
      </main>
      <aside className="communication-right-panel min-h-0 border-t border-slate-200 bg-slate-50/80 p-4 xl:border-l xl:border-t-0">
        {detail && <div className="sticky top-24 max-h-[calc(100vh-140px)] space-y-4 overflow-y-auto pr-1">
          <section className="rounded-[18px] border border-slate-200 bg-white p-4 shadow-sm"><h3 className="font-black text-slate-950">Konuşma Bilgileri</h3><p className="mt-2 text-sm text-slate-700">Durum: <strong>{label(statusOptions, detail.conversation.status)}</strong></p><p className="text-sm text-slate-700">Öncelik: <strong>{label(priorityOptions, detail.conversation.priority)}</strong></p>{detail.conversation.source_customer_conversation_id && <a href={`/hk-admin/iletisim-merkezi?channel=customers&conversation=${detail.conversation.source_customer_conversation_id}`} className="mt-3 inline-flex min-h-10 items-center rounded-[10px] bg-emerald-700 px-4 text-sm font-black text-white shadow-sm hover:bg-emerald-800">Kaynak müşteri konuşması</a>}</section>
          ...
          <section className="rounded-[18px] border border-slate-200 bg-white p-4 shadow-sm"><h3 className="font-black text-slate-950">Geçmiş</h3><button type="button" onClick={() => setHistoryOpen(true)} className="communication-dark-action mt-3 inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-[10px] bg-slate-900 px-4 text-sm font-black text-white shadow-sm hover:bg-slate-800"><Bell size={16} /> Hareket geçmişi</button></section>
        </div>}
      </aside>
    </section>
