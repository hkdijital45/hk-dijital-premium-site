...
export function ReportTools({ report, updates = [], onApplyExtracted, onUpdatesChange }: { report: any; updates?: any[]; onApplyExtracted: (patch: any) => void; onUpdatesChange?: (updates: any[]) => void }) {
  ...
  return <div className="grid gap-3">
    ...
    <details className="rounded-[8px] border border-slate-200 p-3"><summary className="cursor-pointer text-sm font-black">Dışa aktarma</summary><div className="mt-3 flex flex-wrap gap-2">{[["excel", "Excel indir"], ["word", "Word indir"], ["pdf", "PDF indir"]].map(([format, label]) => <a key={format} href={persisted ? `/api/admin/reports/${report.id}/export?format=${format}` : "#"} className={`rounded-full border border-slate-200 px-3 py-2 text-xs ${persisted ? "" : "pointer-events-none opacity-50"}`}>{label}</a>)}</div></details>
