...
function Input({ label, value, onChange, type = "text" }: any) {
  return <label className="grid gap-2 text-sm font-semibold text-slate-700">{label}<input type={type} value={value || ""} onChange={(event) => onChange(event.target.value)} className="min-h-11 rounded-[8px] border border-slate-200 bg-white px-3 text-slate-900" /></label>;
}

function Textarea({ label, value, onChange }: any) {
  return <label className="grid gap-2 text-sm font-semibold text-slate-700">{label}<textarea rows={3} value={value || ""} onChange={(event) => onChange(event.target.value)} className="rounded-[8px] border border-slate-200 bg-white px-3 py-3 text-slate-900" /></label>;
}
...
export function ReportTools({ report, updates = [], onApplyExtracted, onUpdatesChange }: { report: any; updates?: any[]; onApplyExtracted: (patch: any) => void; onUpdatesChange?: (updates: any[]) => void }) {
  ...
  return <div className="grid gap-3">
    ...
    <details className="rounded-[8px] border border-slate-200 p-3"><summary className="cursor-pointer text-sm font-black">Dosyadan aktar</summary><div className="mt-3 grid gap-3"><label className="cursor-pointer rounded-[8px] border border-dashed border-cyan-200/30 p-4 text-sm text-cyan-700">CSV, XLSX veya XLS yükle<input className="hidden" type="file" accept=".csv,.xlsx,.xls,.pdf" onChange={(event) => event.target.files?.[0] && parse(event.target.files[0])} /></label>{preview && <><p className="text-sm text-slate-600">Yüklenen dosyadan çıkarılan veriler: {preview.rowCount} satır. Eksik görünen alanları rapor formunda elle tamamlayabilirsiniz.</p><pre className="max-h-48 overflow-auto rounded-[8px] bg-white p-3 text-xs text-slate-600">{JSON.stringify(preview.metrics, null, 2)}</pre><button onClick={() => onApplyExtracted({ metrics: preview.metrics, time_series: preview.timeSeries, start_date: preview.startDate || report.start_date, end_date: preview.endDate || report.end_date, raw_extracted_data: preview.raw })} className="w-fit rounded-full bg-cyan-300 px-4 py-2 text-xs font-black text-slate-950">Çıkarılan verileri rapora uygula</button></>}</div></details>
    <details className="rounded-[8px] border border-slate-200 p-3"><summary className="cursor-pointer text-sm font-black">Ajans notu ekle</summary><div className="mt-3 grid gap-3 md:grid-cols-2"><Input label="Tarih" type="date" value={note.update_date} onChange={(value: string) => setNote({ ...note, update_date: value })} /><Input label="Başlık" value={note.title} onChange={(value: string) => setNote({ ...note, title: value })} /><label className="grid gap-2 text-sm font-semibold text-slate-700">Kategori<select value={note.category} onChange={(event) => setNote({ ...note, category: event.target.value })} className="min-h-11 rounded-[8px] border border-slate-200 bg-white px-3 text-slate-900">{["Kampanya", "Kreatif", "Pixel / Ölçüm", "Raporlama", "Optimizasyon", "Toplantı / Not"].map((item) => <option key={item}>{item}</option>)}</select></label><label className="grid gap-2 text-sm font-semibold text-slate-700">Durum<select value={note.status} onChange={(event) => setNote({ ...note, status: event.target.value })} className="min-h-11 rounded-[8px] border border-slate-200 bg-white px-3 text-slate-900">{["Planlandı", "Yapıldı", "Devam Ediyor"].map((item) => <option key={item}>{item}</option>)}</select></label><Textarea label="Müşteriye gösterilecek not" value={note.customer_note} onChange={(value: string) => setNote({ ...note, customer_note: value })} /><Textarea label="Sıradaki adım" value={note.next_action} onChange={(value: string) => setNote({ ...note, next_action: value })} /><label className="text-sm"><input type="checkbox" checked={note.is_visible_to_customer} onChange={(event) => setNote({ ...note, is_visible_to_customer: event.target.checked })} /> Müşteriye göster</label><label className="text-sm"><input type="checkbox" checked={note.is_pinned} onChange={(event) => setNote({ ...note, is_pinned: event.target.checked })} /> Üste sabitle</label><button disabled={!persisted} onClick={addNote} className="w-fit rounded-full bg-cyan-300 px-4 py-2 text-xs font-black text-slate-950 disabled:opacity-50">Güncelle</button></div></details>
    {updates.length > 0 && <details className="rounded-[8px] border border-slate-200 p-3"><summary className="cursor-pointer text-sm font-black">Mevcut çalışma günlüğü</summary><div className="mt-3 grid gap-3">{updates.map((item) => <div key={item.id} className="grid gap-3 rounded-[8px] border border-slate-200 bg-slate-50 p-3 md:grid-cols-2"><Input label="Tarih" type="date" value={item.update_date} onChange={(value: string) => updateNote(item, { update_date: value })} /><Input label="Başlık" value={item.title} onChange={(value: string) => updateNote(item, { title: value })} /><label className="grid gap-2 text-sm font-semibold text-slate-700">Kategori<select value={item.category || "Raporlama"} onChange={(event) => updateNote(item, { category: event.target.value })} className="min-h-11 rounded-[8px] border border-slate-200 bg-white px-3 text-slate-900">{["Kampanya", "Kreatif", "Pixel / Ölçüm", "Raporlama", "Optimizasyon", "Toplantı / Not"].map((option) => <option key={option}>{option}</option>)}</select></label><label className="grid gap-2 text-sm font-semibold text-slate-700">Durum<select value={item.status || "Yapıldı"} onChange={(event) => updateNote(item, { status: event.target.value })} className="min-h-11 rounded-[8px] border border-slate-200 bg-white px-3 text-slate-900">{["Planlandı", "Yapıldı", "Devam Ediyor"].map((option) => <option key={option}>{option}</option>)}</select></label><Textarea label="Müşteri notu" value={item.customer_note} onChange={(value: string) => updateNote(item, { customer_note: value })} /><Textarea label="Sıradaki adım" value={item.next_action} onChange={(value: string) => updateNote(item, { next_action: value })} /><button onClick={() => deleteNote(item)} className="w-fit rounded-full border border-red-300/30 px-3 py-2 text-xs text-red-700">Sil</button></div>)}</div></details>}
    <details className="rounded-[8px] border border-slate-200 p-3"><summary className="cursor-pointer text-sm font-black">Premium rapor modülleri</summary><div className="mt-3 grid gap-3">
    ...
    <details className="rounded-[8px] border border-slate-200 p-3"><summary className="cursor-pointer text-sm font-black">Dışa aktarma</summary><div className="mt-3 flex flex-wrap gap-2">{[["excel", "Excel indir"], ["word", "Word indir"], ["pdf", "PDF indir"]].map(([format, label]) => persisted ? <a key={format} href={`/api/admin/reports/${report.id}/export?format=${format}`} className="rounded-full border border-slate-200 px-3 py-2 text-xs">{label}</a> : <button key={format} type="button" disabled title="Önce raporu kaydedin" className="rounded-full border border-slate-200 px-3 py-2 text-xs opacity-50">{label}</button>)}</div></details>
    <details className="rounded-[8px] border border-slate-200 p-3"><summary className="cursor-pointer text-sm font-black">Müşteriye e-posta gönder</summary><div className="mt-3 grid gap-3"><Input label="Konu" value={email.subject} onChange={(value: string) => setEmail({ ...email, subject: value })} /><Textarea label="Mesaj" value={email.message} onChange={(value: string) => setEmail({ ...email, message: value })} /><div className="flex flex-wrap gap-3">{[["pdf", "PDF ekle"], ["excel", "Excel ekle"], ["word", "Word ekle"]].map(([format, label]) => <label key={format} className="text-sm"><input type="checkbox" checked={email.formats.includes(format)} onChange={(event) => setEmail({ ...email, formats: event.target.checked ? [...email.formats, format] : email.formats.filter((item) => item !== format) })} /> {label}</label>)}</div><button disabled={!persisted} onClick={sendEmail} className="w-fit rounded-full bg-cyan-300 px-4 py-2 text-xs font-black text-slate-950 disabled:opacity-50">Müşteriye e-posta gönder</button></div></details>
  </div>;
}
