...
const accentStyles = {
  ...
  amber: "border-amber-100/45 bg-gradient-to-br from-yellow-300 via-amber-400 to-orange-500 text-slate-900 shadow-[0_22px_55px_rgba(245,158,11,.28)]",
  blue: "border-blue-100/45 bg-gradient-to-br from-blue-500 via-sky-500 to-cyan-400 text-slate-900 shadow-[0_22px_55px_rgba(59,130,246,.30)]",
  emerald: "border-emerald-100/45 bg-gradient-to-br from-emerald-400 via-green-500 to-teal-600 text-slate-900 shadow-[0_22px_55px_rgba(16,185,129,.30)]",
  purple: "border-cyan-100/45 bg-gradient-to-br from-sky-500 via-cyan-500 to-blue-600 text-slate-900 shadow-[0_22px_55px_rgba(14,165,233,.24)]",
  orange: "border-orange-100/45 bg-gradient-to-br from-orange-400 via-pink-500 to-rose-500 text-slate-900 shadow-[0_22px_55px_rgba(249,115,22,.30)]",
  pink: "border-sky-100/45 bg-gradient-to-br from-sky-400 via-cyan-500 to-blue-600 text-slate-900 shadow-[0_22px_55px_rgba(14,165,233,.24)]",
  indigo: "border-sky-100/45 bg-gradient-to-br from-sky-600 via-blue-600 to-cyan-600 text-slate-900 shadow-[0_22px_55px_rgba(14,165,233,.24)]",
  yellow: "border-yellow-100/45 bg-gradient-to-br from-yellow-300 via-amber-400 to-orange-400 text-slate-900 shadow-[0_22px_55px_rgba(250,204,21,.32)]",
  gold: "border-yellow-100/45 bg-gradient-to-br from-yellow-300 via-orange-400 to-amber-600 text-slate-900 shadow-[0_22px_55px_rgba(250,204,21,.34)]",
  red: "border-red-100/45 bg-gradient-to-br from-red-500 via-orange-500 to-amber-500 text-slate-900 shadow-[0_22px_55px_rgba(239,68,68,.30)]",
  slate: "border-cyan-100/35 bg-gradient-to-br from-slate-500 via-cyan-600 to-sky-700 text-slate-900 shadow-[0_22px_55px_rgba(14,165,233,.26)]",
  rose: "border-rose-100/45 bg-gradient-to-br from-red-500 via-orange-500 to-amber-500 text-slate-900 shadow-[0_22px_55px_rgba(239,68,68,.24)]",
  teal: "border-teal-100/45 bg-gradient-to-br from-teal-400 via-cyan-500 to-emerald-600 text-slate-900 shadow-[0_22px_55px_rgba(20,184,166,.30)]",
  coral: "border-orange-100/45 bg-gradient-to-br from-orange-400 via-red-400 to-pink-500 text-slate-900 shadow-[0_22px_55px_rgba(251,113,133,.30)]",
  lime: "border-lime-100/45 bg-gradient-to-br from-lime-400 via-green-500 to-emerald-600 text-slate-900 shadow-[0_22px_55px_rgba(132,204,22,.30)]"
};
...
export function MetricCard3D({ label, value, note, accent = "cyan", icon }: { label: string; value: string | number; note?: string; accent?: keyof typeof accentStyles; icon?: ReactNode }) {
  return <motion.div initial={{ opacity: 0, y: 18, rotateX: 5 }} whileInView={{ opacity: 1, y: 0, rotateX: 0 }} viewport={{ once: true, margin: "-60px" }} whileHover={{ y: -8, rotateX: 3, rotateY: -3, scale: 1.015 }} className={`glass-card metric-card-3d relative overflow-hidden border p-5 ${accentStyles[accent]}`}>
    ...
    <div className="flex items-center justify-between gap-3"><p className="text-xs font-black uppercase tracking-[.16em] text-slate-900/75">{label}</p><span className="grid size-9 place-items-center rounded-[8px] border border-white/25 bg-white/15 text-slate-900 shadow-[0_0_24px_rgba(255,255,255,.12)]">{icon || <BarChart3 size={17} />}</span></div>
    <p className="mt-5 text-3xl font-black text-slate-900"><AnimatedKpiCounter value={value} /></p>{note && <p className="mt-3 text-xs leading-5 text-slate-900/72">{note}</p>}
  </motion.div>;
}
...
export function AnimatedChart({ label = "Performans akışı", values = [24, 32, 28, 48, 42, 63, 58, 82] }: { label?: string; values?: number[] }) {
  ...
  return <div className="overflow-hidden rounded-[8px] border border-slate-200 bg-slate-50 p-4">
  ...
  return <div className="flex flex-wrap gap-2">
    {items.map(([name, mark, color], index) => (
      <motion.div
        ...
        className="brand-chip inline-flex min-h-10 items-center gap-2 rounded-full border border-white/12 bg-white/[0.065] px-3 text-[10px] font-black uppercase tracking-[.12em] text-slate-900 shadow-[0_14px_38px_rgba(0,0,0,.2)] backdrop-blur-xl"
      >
  ...
  return <div className="pointer-events-none absolute inset-0 hidden overflow-hidden lg:block">
    {brandEcosystemItems.slice(0, 8).map(([name, mark, color], index) => (
      <motion.div
        ...
        className={`floating-brand-orbit absolute ${positions[index]} rounded-full border border-slate-200 bg-white/[0.055] px-3 py-2 text-[10px] font-black uppercase tracking-[.12em] text-slate-900/80 shadow-[0_18px_60px_rgba(0,0,0,.22)] backdrop-blur-xl`}
      >
...
export function FloatingBadge({ label, className = "", delay = 0 }: { label: string; className?: string; delay?: number }) {
  return <motion.div initial={{ opacity: 0, y: 12, scale: .94 }} animate={{ opacity: 1, y: [0, -9, 0], scale: 1 }} transition={{ opacity: { duration: .4, delay }, y: { duration: 5.2, delay, repeat: Infinity, ease: "easeInOut" } }} className={`absolute rounded-full border border-slate-200 bg-white/[0.08] px-3 py-2 text-[10px] font-black uppercase tracking-[.14em] text-slate-900 shadow-[0_18px_50px_rgba(0,0,0,.24)] backdrop-blur-xl ${className}`}>{label}</motion.div>;
}
...
export function AnimatedMarketingDashboard({ className = "" }: { className?: string }) {
  ...
  return <motion.div initial={{ opacity: 0, y: 44, rotateX: 12, rotateY: -12, scale: .94 }} animate={{ opacity: 1, y: 0, rotateX: 0, rotateY: 0, scale: 1 }} transition={{ duration: .78, ease: [0.16, 1, 0.3, 1] }} whileHover={{ rotateX: 3, rotateY: -5, y: -10, scale: 1.015 }} className={`perspective-scene relative mx-auto max-w-2xl ${className}`}>
    <div className="dashboard-impact-panel relative rounded-[18px] border border-slate-200 bg-white p-3 shadow-[0_36px_120px_rgba(0,0,0,.55),0_0_70px_rgba(34,211,238,.14)]">
      <div className="flex h-7 items-center gap-2 rounded-t-[12px] border border-slate-200 bg-slate-50 px-3">
      ...
      <div className="grid gap-3 rounded-b-[12px] border-x border-b border-slate-200 bg-white p-4 md:grid-cols-[1.15fr_.85fr]">
        <div className="grid gap-3">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="rounded-[8px] border border-orange-200/20 bg-orange-300/[0.08] p-3"><p className="text-xs font-black text-orange-700">Meta Ads</p><p className="mt-2 text-[11px] leading-5 text-slate-600">Kreatif testi ve remarketing akışı hazır.</p></div>
            <div className="rounded-[8px] border border-cyan-200/20 bg-cyan-300/[0.08] p-3"><p className="text-xs font-black text-cyan-700">Google Ads</p><p className="mt-2 text-[11px] leading-5 text-slate-600">Yüksek niyetli aramalar ayrıştırıldı.</p></div>
          </div>
        </div>
        <div className="grid gap-3">
          <div className="rounded-[8px] border border-cyan-200/20 bg-cyan-300/[0.08] p-3"><p className="text-xs font-black text-cyan-700">AI Insight</p><p className="mt-2 text-[11px] leading-5 text-slate-600">Sıcak lead için teklif ve takip önerisi üretildi.</p></div>
          <AnimatedFunnel />
          <div className="rounded-[8px] border border-emerald-200/20 bg-emerald-300/[0.08] p-3"><p className="text-xs font-black text-emerald-700">CRM Lead</p><div className="mt-3 h-2 rounded-full bg-slate-100"><motion.div initial={{ width: "20%" }} animate={{ width: ["42%", "78%", "64%"] }} transition={{ duration: 5, repeat: Infinity, ease: "easeInOut" }} className="h-full rounded-full bg-emerald-300" /></div></div>
        </div>
      </div>
  ...
  return <section className="hero-impact relative min-h-[760px] overflow-hidden border-b border-white/5 px-4 pb-20 pt-20 sm:px-6 lg:px-8 lg:pb-28 lg:pt-28">
    ...
    <div className="relative mx-auto grid max-w-7xl items-center gap-12 lg:grid-cols-[.95fr_1.05fr]">
      <motion.div initial={{ opacity: 0, y: 46, skewY: 1.4 }} animate={{ opacity: 1, y: 0, skewY: 0 }} transition={{ duration: .74, ease: [0.16, 1, 0.3, 1] }}>
        <p className="glitch-label inline-flex rounded-full border border-cyan-200/20 bg-cyan-200/10 px-4 py-2 text-sm font-black text-cyan-700 shadow-[0_0_30px_rgba(34,211,238,.12)]" data-text="HK Dijital Marketing OS">
          HK Dijital Marketing OS · Meta Ads · Google Ads · AI Raporlama
        </p>
        <h1 className="split-reveal mt-7 max-w-4xl text-4xl font-black leading-[1.04] tracking-tight text-slate-900 sm:text-6xl lg:text-7xl">
          {headline}
        </h1>
        <p className="mt-7 max-w-2xl text-base leading-8 text-slate-600 sm:text-lg">{subheadline}</p>
...
export function AIInsightCard({ title, text }: { title: string; text: string }) {
  return <GlassCard className="p-5" interactive><div className="flex items-center gap-3"><BrainCircuit className="text-amber-100" size={20} /><p className="text-xs font-black uppercase tracking-[.16em] text-amber-100">HK Intelligence</p></div><h3 className="mt-5 text-lg font-black text-slate-900">{title}</h3><p className="mt-3 text-sm leading-7 text-slate-600">{text}</p></GlassCard>;
}

export function CRMKanban() {
  return <div className="grid gap-3 sm:grid-cols-3">{[["Yeni Talepler", "12", "cyan"], ["Teklif Süreci", "08", "amber"], ["Takipte", "16", "blue"]].map(([title, count, accent]) => <div key={title} className={`rounded-[8px] border p-4 ${accentStyles[accent as keyof typeof accentStyles]}`}><p className="text-xs font-black uppercase tracking-[.14em]">{title}</p><p className="mt-5 text-3xl font-black text-slate-900">{count}</p><div className="mt-4 space-y-2">{[1, 2].map((item) => <div key={item} className="h-7 rounded-[6px] border border-slate-200 bg-slate-100" />)}</div></div>)}</div>;
}
...
export function CustomerMetricCard({ title, value, help, accent = "cyan" }: { title: string; value: string | number; help: string; accent?: keyof typeof accentStyles }) {
  return <GlassCard className="p-5" interactive><div className="flex items-center justify-between gap-3"><p className="text-sm font-bold text-slate-600">{title}</p><span className={`grid size-9 place-items-center rounded-[8px] border ${accentStyles[accent]}`}><Gauge size={17} /></span></div><p className="mt-5 text-3xl font-black text-slate-900">{value}</p><p className="mt-3 text-xs leading-5 text-slate-400">{help}</p></GlassCard>;
}

export function ReportPreview() {
  return <GlassCard className="p-5"><p className="text-xs font-black uppercase tracking-[.16em] text-cyan-700">Canlı raporlama</p><h3 className="mt-2 text-xl font-black text-slate-900">Kanal performansı tek görünümde</h3><div className="mt-5 grid gap-3 sm:grid-cols-3">{[["ROAS", "4.2x"], ["Potansiyel müşteri", "186"], ["Maliyet", "₺14,8K"]].map(([label, value]) => <div key={label} className="rounded-[8px] border border-slate-200 bg-slate-50 p-3"><p className="text-xs text-slate-400">{label}</p><p className="mt-1 text-lg font-black text-slate-900">{value}</p></div>)}</div><div className="mt-4"><AnimatedChart label="Son 30 gün performansı" /></div></GlassCard>;
}
