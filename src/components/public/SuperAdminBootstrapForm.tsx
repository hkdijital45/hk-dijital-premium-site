  ...
  return (
    <form onSubmit={submit} className="mx-auto grid w-full max-w-xl gap-4 rounded-[8px] border border-white/10 bg-white/[0.06] p-6 shadow-2xl backdrop-blur-xl">
      ...
      <label className="grid gap-2 text-sm font-semibold text-slate-200">
        Ad Soyad
        <input value={fullName} onChange={(event) => setFullName(event.target.value)} required autoComplete="name" className="min-h-12 rounded-[8px] border border-white/10 bg-black/30 px-4 text-white focus:ring-2 focus:ring-cyan-300" />
      </label>
      ...
      <label className="grid gap-2 text-sm font-semibold text-slate-200">
        Bootstrap Secret
        <input type="password" value={bootstrapSecret} onChange={(event) => setBootstrapSecret(event.target.value)} required autoComplete="off" className="min-h-12 rounded-[8px] border border-white/10 bg-black/30 px-4 text-white focus:ring-2 focus:ring-cyan-300" />
      </label>
