...
export function normalizeGoogleAds(input?: Record<string, any>): NormalizedSource {
  ...
  const spend = asNumber(input?.spend || input?.cost || input?.spend_amount);
  const impressions = asNumber(input?.impressions);
  const clicks = asNumber(input?.clicks);
  const ctr = asNumber(input?.ctr, impressions ? (clicks / impressions) * 100 : 0);
  const cpc = asNumber(input?.cpc, clicks ? spend / clicks : 0);
  const cpm = asNumber(input?.cpm, impressions ? (spend / impressions) * 1000 : 0);
  const conversions = asNumber(input?.conversions || input?.results);
  const warnings = [
    ctr > 0 && ctr < 1 ? "CTR (Tıklama Oranı) düşük görünüyor." : "",
    cpc > 25 ? "CPC (Tıklama Başına Maliyet) yüksek." : "",
    connected && !hasMeaningfulValue(input, ["spend", "impressions", "clicks", "conversions"]) ? "Google Ads hesabı bağlı, canlı metrik bekleniyor." : ""
  ].filter(Boolean);
  ...
  return source({
    ...
    metrics: {
      ...
      cpm,
      conversions,
      conversionValue: asNumber(input?.conversionValue || input?.conversion_value),
      campaignCount: asNumber(input?.campaignCount || input?.campaign_count)
    },
    warnings,
...
export function normalizeGA4(input?: Record<string, any>): NormalizedSource {
  ...
  const engagementRate = asNumber(input?.engagementRate || input?.engagement_rate);
  const warnings = [
    engagementRate > 0 && engagementRate < 35 ? "GA4 etkileşim oranı düşük." : "",
    connected && !hasMeaningfulValue(input, ["users", "sessions", "events", "conversions"]) ? "GA4 bağlantısı var, ölçüm verisi bekleniyor." : ""
  ].filter(Boolean);
  ...
  return source({
    ...
    metrics: {
      ...
      sessions: asNumber(input?.sessions),
      engagedSessions: asNumber(input?.engagedSessions || input?.engaged_sessions),
      engagementRate,
      conversions: asNumber(input?.conversions),
      events: asNumber(input?.events),
      topChannels: Array.isArray(input?.topChannels) ? input.topChannels : [],
      topLandingPages: Array.isArray(input?.topLandingPages) ? input.topLandingPages : []
    },
    warnings,
...
export function normalizeSearchConsole(input?: Record<string, any>): NormalizedSource {
  ...
  const averagePosition = asNumber(input?.averagePosition || input?.average_position);
  ...
  return source({
    ...
    metrics: {
      clicks: asNumber(input?.clicks),
      impressions: asNumber(input?.impressions),
    ...
    warnings: connected && !hasMeaningfulValue(input, ["clicks", "impressions", "averagePosition"]) ? ["Search Console bağlantısı var, arama verisi bekleniyor."] : [],
...
export function normalizeGoogleBusinessProfile(input?: Record<string, any>): NormalizedSource {
  ...
  const averageRating = asNumber(input?.averageRating || input?.average_rating);
  const reviewCount = asNumber(input?.reviewCount || input?.review_count);
  ...
  return source({
    ...
    metrics: {
      profileViews: asNumber(input?.profileViews || input?.profile_views),
      searches: asNumber(input?.searches),
      websiteClicks: asNumber(input?.websiteClicks || input?.website_clicks),
      phoneClicks: asNumber(input?.phoneClicks || input?.phone_clicks),
      directionRequests: asNumber(input?.directionRequests || input?.direction_requests),
      reviewCount,
      averageRating
    },
