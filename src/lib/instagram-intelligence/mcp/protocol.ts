...
export const tools: Tool[] = [
  ...
  { name: "create_content_plan", description: "Yazılmış bir içerik planını İçerik Takip'e, YALNIZCA verilen companyId'ye kaydeder (tarih+konu bazında tekrar korumalı — aynı plan iki kez gönderilse bile kayıt çoğalmaz). Her item için scheduled_date ve topic zorunlu; ayrıca üretim detayları kaybolmadan saklansın diye şu opsiyonel alanlar da kabul edilir ve kalıcı notes metnine yazılır: hook, contentFlow, caption, summary, cta, hashtagApproach, goal, audience, priority, rationale, conditions (ör. [DOĞRULANACAK] veya koşullu yayın notları — metin AYNEN korunur), storySupport, productionNotes. companyId zorunludur ve gerçek bir şirkete karşı doğrulanır; geçersiz/çözülememiş companyId ile HİÇBİR satır yazılmaz ve başka bir şirkete (HK Dijital dahil) asla sessizce düşmez. Instagram'a HİÇBİR ŞEY YAYINLAMAZ, sadece İçerik Takip'e planlama satırı ekler.", permission: "WRITE_SAFE", inputSchema: { type: "object", properties: { companyId, items: plan }, required: ["companyId", "items"], additionalProperties: false } },
  { name: "update_content_plan_items", description: "create_content_plan ile zaten oluşturulmuş, VAR OLAN İçerik Takip kayıtlarının eksik üretim detaylarını (hook, contentFlow, caption, summary, cta, hashtagApproach, goal, audience, priority, rationale, conditions, storySupport, productionNotes) tamamlar/günceller — bu alanlar kalıcı notes metnine yeniden yazılır. Her item için id zorunludur. Yalnızca companyId'ye GERÇEKTEN AİT olan id'ler güncellenir (id başka bir şirkete aitse hiçbir şey değişmez, NOT_FOUND döner) — asla yeni satır oluşturmaz/çoğaltmaz, asla scheduled_date/theme/topic/content_format/platforms/is_published değiştirmez. companyId zorunludur.", permission: "WRITE_SAFE", inputSchema: { type: "object", properties: { companyId, items: plan }, required: ["companyId", "items"], additionalProperties: false } },
  ...
  { name: "save_ads_strategy_plan", description: "Claude'un ürettiği yapılandırılmış reklam stratejisini HK Dijital'e YENİ BİR TASLAK (status: draft) olarak kaydeder — asla otomatik onaylanmaz/aktif olmaz. Şema kontrollüdür — eksik/yanlış tipte zorunlu alan reddedilir (hangi alanın eksik olduğu hata mesajında açıkça belirtilir). Hiçbir reklam hesabında değişiklik yapmaz, yalnızca planı kaydeder. Kullanıcı açıkça istemeden ('kaydet', 'HK Dijital'e kaydet') çağırma. `strategy` objesi zorunlu alanlar: companyId (string), dataSources (string[]), dataPeriod: {start, end} (YYYY-MM-DD), confidence ('low'|'medium'|'high'), businessSummary (string, 10+ karakter), metaStrategy: {recommended: boolean, rationale: string, ...isteğe bağlı: objective/audience/placements/creative/cta}, googleStrategy: {recommended: boolean, rationale: string, ...isteğe bağlı: campaignTypes}, thirtyDayPlan: [{phase, description}, ...]. `budget` (ZORUNLU, TAM OLARAK bu alan adlarıyla — yanlış/farklı adlandırma 'budget alanları zorunludur' hatası verir): { hasHistoricalPerformance: boolean, totalMonthlyRecommended: number (aylık TOPLAM önerilen reklam bütçesi, TL), platformSplit: { meta: number, google: number } (yüzde — toplamda ~100; Meta veya Google kullanılmıyorsa değeri 0'dır, alan yine de gönderilmelidir, atlanmaz), rationale: string (bütçe gerekçesi) }. Örnek: budget: { hasHistoricalPerformance: false, totalMonthlyRecommended: 15000, platformSplit: { meta: 100, google: 0 }, rationale: '...' }. Opsiyonel alanlar: assumptions, dataGaps, kpis, risks, implementationGuide, recommendations, ve strateji operasyon kartı için strategyTitle/primaryPlatform/primaryGoal/primaryKpi/campaignSequence/remarketing/internalReport/clientReport.", permission: "WRITE_SAFE", inputSchema: { type: "object", properties: { strategy: { type: "object" } }, required: ["strategy"], additionalProperties: false } },
...
export async function execute(name: string, args: Record<string, unknown>): Promise<unknown> {
  ...
  switch (name) {
    ...
    case "update_content_plan_items": {
      const { validateUpdateItems, updateContentPlanItemDetails, PlanInputError, ContentPlanItemNotFoundError } = await import("@/lib/instagram-intelligence/plan");
      try {
        const items = validateUpdateItems(args.items);
        return await updateContentPlanItemDetails(items, String(args.companyId || ""));
      } catch (error) {
        if (error instanceof PlanInputError) throw new ControlError("INVALID_ARGUMENTS", error.message, 400);
        if (error instanceof ContentPlanItemNotFoundError) throw new ControlError("NOT_FOUND", error.message, 404);
        throw (await toKnownControlError(error)) ?? error;
      }
    }
    ...
    case "update_ads_strategy_content": {
      const { updateAdStrategy, validateAdStrategyPatch, AdStrategyPatchValidationError, AdStrategyNotFoundError } = await import("@/lib/marketing-intelligence/ad-strategies");
      try {
        const patch = validateAdStrategyPatch(args.patch);
        return await updateAdStrategy(String(args.companyId || ""), String(args.strategyId || ""), patch);
      } catch (error) {
