...
export function buildTrendRows(reports: any[], range: CustomerDateRange, platform: CustomerPlatformFilter) {
  ...
  rows.forEach((row) => {
    ...
    const current = grouped.get(key) || { date: isoDate(bucketStart), label, impressions: 0, reach: 0, clicks: 0, conversions: 0, leads: 0, messages: 0, spent: 0, ctr: 0, cpm: 0, average_cpc: 0 };
    const normalized = normalizeMetricRow(row);
    ...
    current.leads += normalized.leads || normalized.conversions;
    current.messages += normalized.messages;
    current.spent += normalized.spent;
    current.ctr = normalized.ctr || (current.impressions > 0 ? (current.clicks / current.impressions) * 100 : 0);
    current.average_cpc = current.clicks > 0 ? current.spent / current.clicks : normalized.average_cpc;
    current.cpm = current.impressions > 0 ? (current.spent / current.impressions) * 1000 : normalized.cpm;
