// Analytics sync. Pulls Instagram Insights at fixed snapshot windows after
// publication and one daily account-level snapshot, storing each as a new
// row (never overwriting) so trend analysis works.
//
// IMPORTANT — verified-metric caveat: Instagram Insights metric names are
// the single fastest-moving part of Meta's API surface. The metric lists
// below reflect the metrics documented as current at the time this was
// written, but this module deliberately does NOT assume they will all
// remain valid forever: fetchResilientMediaMetrics tries the full batch
// first, and on any failure falls back to fetching metrics one at a time,
// keeping whatever succeeds and silently dropping whichever single metric
// Meta has renamed or removed — so one deprecated metric name degrades a
// field, not the entire sync. Re-verify the exact metric list against
// https://developers.facebook.com/docs/instagram-platform/instagram-graph-api/reference/ig-media/insights
// against a real connected account before depending on this for anything
// business-critical.
...
import type { SocialContentItem, MetricSnapshot, SnapshotWindow } from "./types";
...
const WINDOW_HOURS: Record<Exclude<SnapshotWindow, "daily">, number> = { "1h": 1, "24h": 24, "72h": 72, "7d": 24 * 7, "30d": 24 * 30 };
...
async function alreadyCaptured(contentItemId: string, window: SnapshotWindow) {
  const rows = await supabaseRest<Array<{ id: string }>>(`social_metric_snapshots?content_item_id=eq.${encodeURIComponent(contentItemId)}&snapshot_window=eq.${window}&select=id&limit=1`);
...
export async function syncContentAnalytics(): Promise<{ snapshotsWritten: number; itemsChecked: number; errors: string[] }> {
  ...
  try {
    ({ accessToken } = await getUsableInstagramToken());
  } catch (error) {
  ...
  for (const item of publishedItems) {
    ...
    for (const [window, thresholdHours] of Object.entries(WINDOW_HOURS) as Array<[Exclude<SnapshotWindow, "daily">, number]>) {
      ...
      try {
        ...
        const snapshot: Partial<MetricSnapshot> = {
          content_item_id: item.id, snapshot_scope: "content", snapshot_window: window,
        ...
        const score = computeContentScore(snapshot as MetricSnapshot, item.cta_goal);
        const created = await supabaseRest<MetricSnapshot[]>("social_metric_snapshots", { method: "POST", body: JSON.stringify({ workspace_id: SOCIAL_WORKSPACE_ID, ...snapshot, performance_score: score }) });
...
export async function syncAccountSnapshot(): Promise<{ ok: boolean; error?: string }> {
  try {
    ...
    await supabaseRest("social_metric_snapshots", {
      method: "POST",
      body: JSON.stringify({ workspace_id: SOCIAL_WORKSPACE_ID, content_item_id: null, snapshot_scope: "account", snapshot_window: "daily", reach: values.reach || 0, follower_count: values.follower_count || null, captured_at: new Date().toISOString() })
    });
    await recordInstagramSuccess({ last_insights_sync_at: new Date().toISOString() });
    return { ok: true };
  } catch (error) {
