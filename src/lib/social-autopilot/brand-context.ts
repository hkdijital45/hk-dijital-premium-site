...
// built on (spec section 35: "reuse shared brand context"). Kept as a single
// function so brand rules — especially the absolute privacy rule — are
// defined once, not copy-pasted into a dozen prompt strings.
...
export function buildBrandContextBlock(brand: SocialBrandProfile): string {
  return [
    `Marka: ${brand.brand_name} — bir dijital pazarlama ajansı.`,
    `Misyon: ${brand.mission}`,
    `Hizmet alanları: ${brand.service_categories.join(", ")}`,
    `Coğrafya: ${brand.geography}`,
    `Ton: ${brand.tone_guidelines}`,
    `Görsel stil notları: ${brand.visual_style_notes}`,
...
1. Gerçek müşteri adı, kullanıcı adı, marka adı, alan adı, telefon, e-posta veya özel kampanya verisi ASLA kullanma. Örnek gerekiyorsa yalnızca şu tarz anonim tanımlar kullan: ${ANONYMIZED_EXAMPLE_DESCRIPTORS.join(", ")}.
2. Uydurma istatistik, uydurma araştırma, uydurma "%X artış" sonucu, uydurma müşteri yorumu veya uydurma vaka çalışması ASLA üretme. Somut bir rakam veriyorsan bunun genel/eğitim amaçlı bir örnek olduğunu ima et, gerçek bir müşteri sonucuymuş gibi sunma.
3. İçeriğin "AI tarafından yazıldığı asla belli olmayacak" gibi bir vaat YOK — bunun yerine güçlü, doğal, tecrübeli bir uzmanın sesiyle yaz.
