// Modular system prompts (spec section 35) — one focused prompt per job
// instead of one fragile mega-prompt, each built on the shared brand context
// block, versioned via PROMPT_VERSION (stored on every generated row so a
// future prompt change is traceable against historical content/strategies).
//
// Only the prompts the engine actually calls are implemented as real AI
// calls: social-strategist, content-writer (reel/carousel/static/story share
// one schema-driven prompt, parameterized by content_type), a combined
// brand-editor+quality-reviewer pass, and an analytics-strategist synthesis
// pass for learning summaries. Privacy, factuality-claim-shape, duplicate,
// CTA/platform-fit and grammar checks are deterministic local logic in
// quality-gate.ts — spec section 36 (token/cost control) explicitly asks to
// avoid unnecessary AI calls, and a regex/heuristic check is both cheaper
// and more reliably enforced than asking a model to police itself.
import type { SocialBrandProfile, SocialContentType, SocialFunnelStage } from "./types";
...
export function buildStrategyPrompt(params: {
  ...
  const system = `Sen HK Social Autopilot'un "social-strategist" modülüsün — deneyimli bir dijital pazarlama ajansının sosyal medya stratejistisin. Görevin, ${params.brand.brand_name} için Instagram hesabında takipçi kalitesi, otorite, lead üretimi ve organik erişimi artıracak, birbirine bağlı 30 günlük bir içerik stratejisi tasarlamak — 30 alakasız gönderi değil, VALUE -> TRUST -> AUTHORITY -> NEED -> CONVERSION akışıyla ilerleyen tutarlı bir plan.
...
function contentTypeSchema(contentType: SocialContentType) {
  if (contentType === "reel") {
    return `{
...
export function buildContentWriterPrompt(brief: ContentBrief): PromptPair {
  const roleByType: Record<SocialContentType, string> = {
    reel: "reel-writer",
    carousel: "carousel-writer",
    static: "brand-editor",
    story: "caption-editor"
  };
  const system = `Sen HK Social Autopilot'un "${roleByType[brief.contentType]}" modülüsün — tecrübeli bir ajans metin yazarısın. Instagram için Türkçe, doğal, tecrübeli-insan sesiyle "${brief.contentType}" formatında içerik üretiyorsun.
...
SADECE şu JSON şemasında yanıt ver:
${contentTypeSchema(brief.contentType)}`;
...
// ---------------------------------------------------------------------------
// brand-editor + quality-reviewer (combined AI quality pass)
// ---------------------------------------------------------------------------

export function buildQualityReviewPrompt(params: { brand: SocialBrandProfile; contentType: SocialContentType; caption: string; hook: string; script?: string }): PromptPair {
  const system = `Sen HK Social Autopilot'un birleşik "brand-editor" ve "quality-reviewer" modülüsün. Görevin, üretilmiş bir Instagram içeriğini yayına çıkmadan önce editoryal kalite açısından denetlemek — bir ajans editörünün yapacağı gibi.
...
- cta_quality: CTA net, tek ve funnel aşamasına uygun mu? ("DM gönderin" gibi klişe/tekrarlayan CTA'dan kaçınılmış mı?)
...
// ---------------------------------------------------------------------------
// analytics-strategist (learning synthesis — reasoning text only, never the
...
export function buildLearningSynthesisPrompt(statsSummary: string): PromptPair {
  const system = `Sen HK Social Autopilot'un "analytics-strategist" modülüsün. Sana önceden hesaplanmış, gerçek Instagram performans istatistikleri özeti veriliyor. Görevin ham sayıları YORUMLAMAK — uydurma sayı üretme, sadece verilen istatistiklere dayanarak kısa, uygulanabilir bir strateji önerisi yaz.
...
// ---------------------------------------------------------------------------
// monthly-optimizer — reuses buildStrategyPrompt directly (spec explicitly
// treats the monthly refresh as "generate next 30-day strategy informed by
// learnings", i.e. the same social-strategist job with real evidence this
// time instead of cold-start assumptions).
// ---------------------------------------------------------------------------
