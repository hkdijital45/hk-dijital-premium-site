...
// actual publish-time precheck + Instagram API call. A cron tick running
// twice must never publish the same content twice — the claim step below
// is a conditional single-row UPDATE (status=eq.scheduled in the filter),
// which PostgREST executes atomically; if a second runner races the same
// row, its conditional UPDATE matches zero rows and it moves on, rather
// than both runners proceeding to publish.
...
export async function enqueueContentItem(contentItemId: string, scheduledAt: string): Promise<SocialQueueItem> {
  ...
  if (existing[0] && !["scheduled", "cancelled"].includes(existing[0].status)) throw new Error("Kuyruk öğesi aktif, yayınlanmış veya inceleme bekleyen durumdayken sıfırlanamaz.");
  ...
  const row = existing.length
    ? (await supabaseRest<SocialQueueItem[]>(`social_publish_queue?content_item_id=eq.${encodeURIComponent(contentItemId)}&status=in.(scheduled,cancelled)&select=*`, { method: "PATCH", body: JSON.stringify(patch) }))[0]
    : (await supabaseRest<SocialQueueItem[]>("social_publish_queue", { method: "POST", body: JSON.stringify({ workspace_id: SOCIAL_WORKSPACE_ID, content_item_id: contentItemId, idempotency_key: `content-${contentItemId}`, ...patch }) }))[0];

  if (!row) throw new Error("Kuyruk eşzamanlı olarak değişti; istek reddedildi.");
  await supabaseRest(`social_content_items?id=eq.${encodeURIComponent(contentItemId)}`, { method: "PATCH", body: JSON.stringify({ publication_status: "scheduled", scheduled_at: scheduledAt }) });
  return row;
}

export async function cancelQueueItem(contentItemId: string) {
  const rows = await supabaseRest<SocialQueueItem[]>(`social_publish_queue?content_item_id=eq.${encodeURIComponent(contentItemId)}&status=eq.scheduled&select=*`, { method: "PATCH", body: JSON.stringify({ status: "cancelled" }) });
  if (!rows.length) throw new Error("İptal edilebilir planlı kuyruk öğesi yok.");
...
async function claimQueueRow(row: SocialQueueItem, lockedBy: string): Promise<SocialQueueItem | null> {
  const rows = await supabaseRest<SocialQueueItem[]>(`social_publish_queue?id=eq.${encodeURIComponent(row.id)}&status=eq.scheduled&scheduled_at=eq.${encodeURIComponent(row.scheduled_at)}&select=*`, {
    method: "PATCH", body: JSON.stringify({ status: "preparing", locked_at: new Date().toISOString(), locked_by: lockedBy })
  });
  return rows[0] || null;
}
...
async function releaseForRetry(queueItem: SocialQueueItem, error: unknown, category: string) {
  ...
  if (nextAttempt >= queueItem.max_attempts) {
    ...
    await supabaseRest(`social_content_items?id=eq.${encodeURIComponent(queueItem.content_item_id)}`, { method: "PATCH", body: JSON.stringify({ publication_status: "needs_review", failure_reason: message.slice(0, 500) }) });
    return "needs_review" as const;
  }
  const nextAttemptAt = new Date(Date.now() + backoffMinutes(nextAttempt) * 60_000).toISOString();
  await supabaseRest(`social_publish_queue?id=eq.${encodeURIComponent(queueItem.id)}`, {
    method: "PATCH", body: JSON.stringify({ status: "scheduled", attempt_count: nextAttempt, next_attempt_at: nextAttemptAt, last_error: message.slice(0, 500), error_category: category, locked_at: null, locked_by: null })
  });
...
async function markPermanentFailure(queueItem: SocialQueueItem, reason: string) {
  ...
  await supabaseRest(`social_content_items?id=eq.${encodeURIComponent(queueItem.content_item_id)}`, { method: "PATCH", body: JSON.stringify({ publication_status: "needs_review", failure_reason: reason.slice(0, 500) }) });
}

/** Publish-time precheck — revalidates everything right before the API
 * call, not just at schedule time, since state can change between
 * scheduling and the scheduled moment (autopilot paused, quality score
 * invalidated by a manual edit, Instagram disconnected, media not yet
 * uploaded, etc). */
async function precheckContentItem(item: SocialContentItem, settings: SocialAutopilotSettings | null): Promise<{ ok: true } | { ok: false; permanent: boolean; reason: string }> {
  ...
  if (!publicationAllowed(settings)) return { ok: false, permanent: false, reason: "Üretim ortamında yayın devre dışı veya test modu aktif." };
  ...
  if ((item.media_quality_score ?? 0) < settings.min_media_quality_score) return { ok: false, permanent: true, reason: "Medya kalite eşiği karşılanmadı." };
  if (!item.caption?.trim()) return { ok: false, permanent: true, reason: "Caption boş." };
  if (item.media_generation_status !== "generated" && item.media_generation_status !== "not_applicable") {
    return { ok: false, permanent: true, reason: "Medya hazır değil (MANUAL MEDIA bekleniyor veya AUTO MEDIA üretimi tamamlanmadı)." };
  }
  if (!item.media_asset_urls?.length) return { ok: false, permanent: true, reason: "Medya dosyası yok — İçerik Stüdyosu'ndan görsel/video yükleyin (MANUAL MEDIA) veya bir AUTO MEDIA sağlayıcısı yapılandırın." };
...
async function publishToInstagram(item: SocialContentItem, accessToken: string, igUserId: string) {
  if (item.content_type === "carousel") {
    ...
    for (const url of item.media_asset_urls) {
      ...
      const child = await createMediaContainer(accessToken, igUserId, { mediaType: isVideo ? "VIDEO" : "IMAGE", isCarouselItem: true, ...(isVideo ? { videoUrl: url } : { imageUrl: url }) });
    ...
    return publishMediaContainer(accessToken, igUserId, parent.id);
  }
...
export async function processDueQueue(runId: string, contentItemId?: string): Promise<QueueProcessSummary> {
  const summary: QueueProcessSummary = { checked: 0, published: 0, retried: 0, needsReview: 0, skipped: 0 };
  const settings = await getSettings();
  if (!publicationAllowed(settings)) return summary;
  const now = new Date().toISOString();

  const dueRows = await supabaseRest<SocialQueueItem[]>(
    `social_publish_queue?workspace_id=eq.${SOCIAL_WORKSPACE_ID}&${contentItemId ? `content_item_id=eq.${encodeURIComponent(contentItemId)}&` : ""}status=eq.scheduled&scheduled_at=lte.${encodeURIComponent(now)}&or=(next_attempt_at.is.null,next_attempt_at.lte.${encodeURIComponent(now)})&select=*&order=scheduled_at.asc&limit=10`
  );

  for (const row of dueRows) {
    summary.checked += 1;
    const claimed = await claimQueueRow(row, runId);
    ...
    try {
      ...
      // Re-read pause/test settings immediately before the external side effect.
      if (!publicationAllowed(await getSettings())) throw new Error("Preflight sırasında yayın devre dışı bırakıldı.");
      ...
      await supabaseRest("social_publications", { method: "POST", body: JSON.stringify({ content_item_id: item.id, external_media_id: published.id, external_permalink: permalink?.permalink || null, media_type: item.content_type, published_at: publishedAt }) });
      await supabaseRest(`social_content_items?id=eq.${encodeURIComponent(item.id)}`, { method: "PATCH", body: JSON.stringify({ publication_status: "published", published_at: publishedAt, external_media_id: published.id, external_permalink: permalink?.permalink || null, failure_reason: null }) });
      ...
      // A timeout or persistence failure after starting publication has an
      // unknown outcome. Never automatically replay a potentially successful
      // remote publish; reconcile in Instagram before any manual retry.
      if (publicationAttempted) {
        await markPermanentFailure(claimed, "Yayın sonucu belirsiz; yeniden denemeden önce Instagram'da doğrulayın.");
