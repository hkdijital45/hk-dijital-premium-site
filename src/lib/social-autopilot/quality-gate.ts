// Multi-stage content quality gate. Combines cheap local deterministic
// checks (privacy, cliché, duplicate, platform-fit, SEO) with one AI pass
// covering the four dimensions that genuinely need editorial judgement
// (brand fit, natural language, CTA quality, grammar) — see prompts.ts.
// Privacy is a hard, non-negotiable gate: any real-data leak caps the score
// regardless of every other dimension. The AI pass is fully OPTIONAL — see
// allowRuntimeAi below — local checks alone still gate the item when it's
// unavailable/disabled, exactly as if the call had been attempted and
// failed.
...
import { scanForPrivacyLeaks, type PrivacyWatchlist, type PrivacyLeakHit } from "./privacy-scan";
import type { SocialAiProviderPreference, SocialBrandProfile, ClicheEntry, SocialContentItem, QualityCheckResult } from "./types";
...
function checkPrivacy(item: SocialContentItem, watchlist: PrivacyWatchlist): { result: CheckResult; hits: PrivacyLeakHit[] } {
  ...
  return {
    hits,
    result: hits.length
      ? { passed: false, score: 0, detail: `Olası korunan veri tespit edildi: ${hits.map((hit) => `${hit.type}:${hit.value}`).join(", ")}` }
      : { passed: true, score: 100, detail: "Korunan veri tespit edilmedi." }
  };
}

function checkCliche(item: SocialContentItem, blacklist: ClicheEntry[]): { result: CheckResult; hits: ClicheHit[] } {
  ...
  return {
    ...
    result: {
      passed: score >= 70,
      score,
      detail: hits.length || warnings.length ? `Klişe/AI izi tespit edildi: ${[...hits.map((hit) => hit.phrase), ...warnings].join("; ")}` : "Klişe ifade tespit edilmedi."
    }
  };
}
...
function checkDuplicate(item: SocialContentItem, recent: Array<Pick<SocialContentItem, "hook" | "caption" | "creative_brief">>): CheckResult {
  ...
  return {
    ...
    score,
    detail: maxSimilarity >= 0.55 ? `Son içeriklerle yüksek benzerlik (%${Math.round(maxSimilarity * 100)}) — özgünlük düşük.` : "Son içeriklerle önemli bir benzerlik tespit edilmedi."
  };
}
...
export async function runQualityGate(params: {
  ...
  // Gates the one AI call this function ever makes. false in NO_RUNTIME_AI/
  // CLAUDE_CODE_ASSISTED mode — the four AI-editorial dimensions fall back
  // to their documented default pass score exactly as if the call had been
  // attempted and failed.
  allowRuntimeAi: boolean;
}): Promise<QualityCheckResult & { aiUnavailable: boolean }> {
  ...
  const reviewPrompt = buildQualityReviewPrompt({ brand, contentType: item.content_type, caption: item.caption, hook: item.hook, script: item.creative_brief?.script });
  ...
  if (params.allowRuntimeAi) {
    try {
      const result = await generateSocialContent({ action: "social_autopilot_quality_review", systemPrompt: reviewPrompt.system, prompt: reviewPrompt.prompt, preference: params.aiPreference, allowRuntimeAi: true, expectedOutputSize: "short" });
      aiReview = parseStructuredJson(result.text, isAiReview);
    } catch {
      aiUnavailable = true; // AI editorial pass unavailable this attempt — local checks still gate the item.
    }
  } else {
    aiUnavailable = true; // not attempted at all — this mode never calls a runtime AI endpoint.
  }
  ...
  let overall = Math.round(Object.entries(weights).reduce((sum, [key, weight]) => checks[key] ? sum + checks[key].score * weight : sum, 0) / (totalWeight || 1));
  if (!privacy.result.passed) overall = Math.min(overall, 15);

  const passed = privacy.result.passed && Object.values(checks).every((check) => check.passed || check === checks.client_data_leak);

  return { content_item_id: item.id, overall_score: overall, checks, passed, attempt, aiUnavailable };
}
