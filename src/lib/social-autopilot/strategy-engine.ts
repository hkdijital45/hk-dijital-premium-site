// Rolling 30-day strategy generation (spec sections 9, 29). Produces one
// coherent monthly plan — not 30 unrelated posts — informed by the previous
// period's real results once they exist (see learning-engine.ts).
...
import { SOCIAL_WORKSPACE_ID, type SocialAiProviderPreference, type SocialBrandProfile, type SocialStrategy } from "./types";
...
function isStrategyAiOutput(value: unknown): value is StrategyAiOutput {
  ...
  return typeof candidate.monthly_objective === "string" &&
    typeof candidate.target_audience === "string" &&
    Array.isArray(candidate.content_pillars) &&
    Array.isArray(candidate.creative_themes);
}
...
export async function getActiveStrategy(): Promise<SocialStrategy | null> {
  const rows = await supabaseRest<SocialStrategy[]>(
    `social_strategies?workspace_id=eq.${SOCIAL_WORKSPACE_ID}&status=eq.active&select=*&order=period_start.desc&limit=1`
  );
  return rows[0] || null;
}
...
export async function getMostRecentStrategy(): Promise<SocialStrategy | null> {
  const rows = await supabaseRest<SocialStrategy[]>(
    `social_strategies?workspace_id=eq.${SOCIAL_WORKSPACE_ID}&select=*&order=period_start.desc&limit=1`
  );
  return rows[0] || null;
}
...
export async function generateMonthlyStrategy(params: {
...
}): Promise<{ strategy: SocialStrategy; usedDemo: boolean }> {
  ...
  if (previous && previous.status === "active") {
    await supabaseRest(`social_strategies?id=eq.${encodeURIComponent(previous.id)}`, {
  ...
  const rows = await supabaseRest<SocialStrategy[]>("social_strategies", {
    ...
    body: JSON.stringify({
      ...
      status: "active",
      monthly_objective: parsed.monthly_objective,
      target_audience: parsed.target_audience,
      funnel_distribution: parsed.funnel_distribution || {},
      content_pillars: parsed.content_pillars || [],
      posting_frequency: parsed.posting_frequency || 5,
      reel_ratio: parsed.reel_ratio ?? 0.5,
      carousel_ratio: parsed.carousel_ratio ?? 0.3,
      static_ratio: parsed.static_ratio ?? 0.2,
      story_strategy: parsed.story_strategy || "",
      follower_strategy: parsed.follower_strategy || "",
      authority_strategy: parsed.authority_strategy || "",
      lead_strategy: parsed.lead_strategy || "",
      conversion_strategy: parsed.conversion_strategy || "",
      community_strategy: parsed.community_strategy || "",
      testing_hypotheses: parsed.testing_hypotheses || [],
  ...
  return { strategy: rows[0], usedDemo: result.provider === "demo" };
}
